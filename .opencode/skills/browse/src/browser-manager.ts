...
export interface BrowserState {
  ...
  pages: Array<{
    url: string
    isActive: boolean
    storage: { localStorage: Record<string, string | null>; sessionStorage: Record<string, string | null> } | null
  }>
}
...
export class BrowserManager {
  ...
  async saveState(): Promise<BrowserState> {
    ...
    for (const [id, page] of this.pages) {
      const url = page.url()
      let storage: BrowserState["pages"][0]["storage"] = null
      try {
        const result = (await page.evaluate(() => {
          const extract = (s: Storage) => Object.fromEntries(Object.keys(s).map((k) => [k, s.getItem(k) ?? null]))
          return { localStorage: extract(localStorage), sessionStorage: extract(sessionStorage) }
        })) as unknown as { localStorage: Record<string, string | null>; sessionStorage: Record<string, string | null> }
        storage = result
      } catch {}
      pages.push({
