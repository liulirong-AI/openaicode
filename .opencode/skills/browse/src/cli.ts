...
async function startServer(): Promise<ServerState> {
  ...
  const serverCmd = useNode ? ["node", NODE_SERVER_SCRIPT] : ["bun", "run", SERVER_SCRIPT]
  ...
  // Wait for server to be fully initialized (health check passing)
...
async function ensureServer(): Promise<ServerState> {
  ...
  if (state) {
    ...
    // Try health check first (works even if process check fails on Windows)
    try {
      const resp = await fetch(`http://127.0.0.1:${state.port}/health`, {
        signal: AbortSignal.timeout(2000),
      })
    ...
    // Process might still be alive even if isProcessAlive returns false (Windows quirk)
    if (!isProcessAlive(state.pid)) {
  ...
  try {
    ...
    if (freshState && isProcessAlive(freshState.pid)) {
      ...
      try {
        const resp = await fetch(`http://127.0.0.1:${freshState.port}/health`, {
          signal: AbortSignal.timeout(2000),
        })
        if (resp.ok) {
          const health = (await resp.json()) as any
