...
export async function handleWriteCommand(command: string, args: string[], bm: BrowserManager): Promise<string> {
  ...
  switch (command) {
    ...
    case "device": {
      const device = args[0]
      if (!device) {
        const available = bm.getAvailableDevices().join(", ")
        return `Available devices: ${available}`
      }
      if (device === "--list") {
        const devices = bm
          .getAvailableDevices()
          .map((d) => {
            const info = bm.getDevice() === d ? " (current)" : ""
            return `  ${d}${info}`
          })
          .join("\n")
        return `Available devices:\n${devices}`
      }
      const result = await bm.setDevice(device)
      return result
    }
    ...
    case "block": {
      const patterns = args
        ...
        .map((p) => p.trim())
        .filter(Boolean)
      if (patterns.length === 0) throw new Error("Usage: browse block <pattern1,pattern2,...>")
      await bm.setRequestBlock(patterns)
      return `Blocking requests matching: ${patterns.join(", ")}`
    }

    case "mock": {
      const [urlPattern, ...responseParts] = args
      if (!urlPattern || responseParts.length === 0) {
        throw new Error("Usage: browse mock <url-pattern> <response-body>")
      }
      const response = responseParts.join(" ")
      await bm.setRequestMock(urlPattern, response)
      return `Mocking responses for: ${urlPattern}`
    }

    case "unmock": {
      await bm.clearRequestMocks()
      return "Cleared all request mocks and blocks"
    }

    case "cookies-save": {
      const [path] = args
      const cookiePath = path || ".gstack/browse-cookies.json"
      if (!bm.getContext()) throw new Error("Browser not launched")
      const cookies = await bm.getContext().cookies()
      const dir = cookiePath.includes("/") ? cookiePath.substring(0, cookiePath.lastIndexOf("/")) : "."
      if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true })
      fs.writeFileSync(cookiePath, JSON.stringify(cookies, null, 2))
      return `Saved ${cookies.length} cookies to ${cookiePath}`
    }
    ...
    case "cookies-load": {
      ...
      await bm.getContext().addCookies(cookies)
      return `Loaded ${cookies.length} cookies from ${cookiePath}`
    }
