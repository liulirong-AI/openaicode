...
**If NO runtime detected** (no config files found): Use the question tool:
...
6. **If ASK items remain,** present them in ONE question:
   - List each with number, severity, problem, recommended fix
   - Per-item options: A) Fix  B) Skip
   - Overall RECOMMENDATION
   - If 3 or fewer ASK items, you may use individual question tool calls instead
...
**VALID & ACTIONABLE:** Use the question tool with:
...
**FALSE POSITIVE:** Use the question tool:
