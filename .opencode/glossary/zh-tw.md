...
- `OpenAiCode` (preserve casing in prose; keep `opencode` only when it is part of commands, package names, paths, or code)
- `OpenAiCode Zen`
- `OpenAiCode CLI`
- `CLI`, `TUI`, `MCP`, `OAuth`
...
- Avoid `opencode` in prose when referring to the product name; use `OpenAiCode`
