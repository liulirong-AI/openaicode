...
You can add external tools to OpenAiCode using the _Model Context Protocol_, or MCP. OpenAiCode supports both local and remote servers.
...
You can define MCP servers in your [OpenAiCode Config](https://opencode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
OpenAiCode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, OpenAiCode will:
...
If the server requires authentication, OpenAiCode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `opencode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, OpenAiCode will store the tokens securely in `~/.local/share/opencode/mcp-auth.json`.
...
Your MCPs are available as tools in OpenAiCode, alongside built-in tools. So you can manage them through the OpenAiCode config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect OpenAiCode to your Sentry account.
