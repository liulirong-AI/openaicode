...
OpenAiCode Go is a low cost subscription — **$5 for your first month**, then **$10/month** — that gives you reliable access to popular open coding models.

:::note
OpenAiCode Go is currently in beta.
:::

Go works like any other provider in OpenAiCode. You subscribe to OpenAiCode Go and
...
OpenAiCode Go gives you access to these models for **$5 for your first month**, then **$10/month**.

---

## How it works

OpenAiCode Go works like any other provider in OpenAiCode.

1. You sign in to **<a href={console}>OpenAiCode Zen</a>**, subscribe to Go, and
   copy your API key.
2. You run the `/connect` command in the TUI, select `OpenAiCode Go`, and paste
   your API key.
3. Run `/models` in the TUI to see the list of models available through Go.

:::note
Only one member per workspace can subscribe to OpenAiCode Go.
:::
...
OpenAiCode Go includes the following limits:
...
4. Have **no lock-in** by allowing you to use any other provider with OpenAiCode as well.
