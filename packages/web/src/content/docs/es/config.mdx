---
title: Configuración
description: Usando la configuración OpenAiCode JSON.
---

Puede configurar OpenAiCode usando un archivo de configuración JSON.

---

## Formato

OpenAiCode admite los formatos **JSON** y **JSONC** (JSON con comentarios).
...
Coloque su configuración global OpenAiCode en `~/.config/opencode/opencode.json`. Utilice la configuración global para las preferencias de todo el usuario, como temas, proveedores o combinaciones de teclas.
...
Cuando se inicia OpenAiCode, busca un archivo de configuración en el directorio actual o recorre hasta el directorio Git más cercano.
...
- `mdns`: habilita el descubrimiento de servicios mDNS. Esto permite que otros dispositivos en la red descubran su servidor OpenAiCode.
...
Puede configurar los proveedores y modelos que desea utilizar en su configuración OpenAiCode a través de las opciones `provider`, `model` y `small_model`.
...
La opción `small_model` configura un modelo separado para tareas livianas como la generación de títulos. De forma predeterminada, OpenAiCode intenta utilizar un modelo más económico si su proveedor tiene uno disponible; de ​​lo contrario, recurre a su modelo principal.
...
Puede configurar el tema que desea usar en su configuración OpenAiCode a través de la opción `theme`.
...
El agente predeterminado debe ser un agente principal (no un subagente). Puede ser un agente integrado como `"build"` o `"plan"`, o un [agente personalizado](/docs/agents) que haya definido. Si el agente especificado no existe o es un subagente, OpenAiCode recurrirá a `"build"` con una advertencia.
...
OpenAiCode descargará automáticamente cualquier actualización nueva cuando se inicie. Puede desactivar esto con la opción `autoupdate`.
...
Los [complementos](/docs/plugins) amplían OpenAiCode con herramientas, enlaces e integraciones personalizados.
...
Esto es útil cuando desea restringir OpenAiCode para que utilice únicamente proveedores específicos en lugar de deshabilitarlos uno por uno.
