---
title: Proveedores
description: Usando cualquier proveedor LLM en OpenAiCode.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenAiCode usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración OpenAiCode.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu OpenAiCode
configuración.
...
OpenAiCode Zen es una lista de modelos proporcionados por el equipo OpenAiCode que han sido
probado y verificado para funcionar bien con OpenAiCode. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con OpenAiCode Zen.
:::
...
Funciona como cualquier otro proveedor en OpenAiCode y su uso es completamente opcional.

---

## OpenAiCode Go

OpenAiCode Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de OpenAiCode que han sido
probado y verificado para funcionar bien con OpenAiCode.

1. Ejecute el comando `/connect` en TUI, seleccione `OpenAiCode Go` y diríjase a [opencode.ai/auth](https://opencode.ai/zen).
...
Para usar Amazon Bedrock con OpenAiCode:
...
:::info
El uso de su suscripción Claude Pro/Max en OpenAiCode no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
2. Elija su método de autenticación:
   ...
   3. Nombre: `OpenAiCode`, Alcances: `api`
...
OpenAiCode utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear OpenAiCode
...
Para utilizar Google Vertex AI con OpenAiCode:
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) para registrar automáticamente cada conversación OpenAiCode como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación OpenAiCode enumerada como una sesión separada.
...
:::tip
Ollama puede configurarse automáticamente para OpenAiCode. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/opencode) para obtener más detalles.
:::
...
Para usar Ollama Cloud con OpenAiCode:
...
3. Copie la clave API para usarla en OpenAiCode.
...
6. **Importante**: Antes de usar modelos en la nube en OpenAiCode, debe obtener la información del modelo localmente:
...
OpenAiCode Zen es una lista de modelos probados y verificados proporcionada por el equipo OpenAiCode. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>OpenAiCode Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **OpenAiCode Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave OpenAiCode API.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con OpenAiCode:
...
Los campos `limit` le permiten a OpenAiCode comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
