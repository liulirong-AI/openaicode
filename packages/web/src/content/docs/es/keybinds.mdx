...
OpenAiCode tiene una lista de combinaciones de teclas que puede personalizar a través de `tui.json`.
...
OpenAiCode usa una tecla `leader` para la mayoría de las combinaciones de teclas. Esto evita conflictos en tu terminal.
...
La entrada de solicitud de la aplicación de escritorio OpenAiCode admite atajos comunes de estilo Readline/Emacs para editar texto. Estos están integrados y actualmente no se pueden configurar a través de `opencode.json`.
