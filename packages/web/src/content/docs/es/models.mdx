...
Los proveedores más populares están precargados de forma predeterminada. Si agregó las credenciales de un proveedor mediante el comando `/connect`, estarán disponibles cuando inicie OpenAiCode.
...
Aqui tienes varios modelos que funcionan bien con OpenAiCode, sin orden particular. (Esta no es una lista exhaustiva ni necesariamente actualizada):
...
Aquí el ID completo es `provider_id/model_id`. Por ejemplo, si usa [OpenAiCode Zen](/docs/zen), usaría `opencode/gpt-5.1-codex` para GPT 5.1 Codex.
...
Muchos modelos admiten múltiples variantes con diferentes configuraciones. OpenAiCode se envía con variantes predeterminadas integradas para proveedores populares.

### Variantes integradas

OpenAiCode se envía con variantes predeterminadas para muchos proveedores:
...
Cuando se inicia OpenAiCode, busca modelos en el siguiente orden de prioridad:
...
2. La lista de modelos en la configuración OpenAiCode.
