---
title: CLI
description: Opciones y comandos de la CLI de OpenAiCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI de OpenAiCode de forma predeterminada inicia el [TUI](/docs/tui) cuando se ejecuta sin ningún argumento.

```bash
opencode
```

Pero también acepta comandos como se documenta en esta página. Esto le permite interactuar con OpenAiCode mediante programación.
...
Inicie la interfaz de usuario del terminal OpenAiCode.
...
La CLI de OpenAiCode también tiene los siguientes comandos.

---

### agent

Administrar agentes para OpenAiCode.
...
Conecte una terminal a un servidor backend OpenAiCode que ya se esté ejecutando y iniciado mediante los comandos `serve` o `web`.

```bash
opencode attach [url]
```

Esto permite usar TUI con un backend remoto OpenAiCode. Por ejemplo:
...
OpenAiCode funciona con la lista de proveedores en [Models.dev](https://models.dev), por lo que puede usar `opencode auth login` para configurar las claves API para cualquier proveedor que desee utilizar. Esto se almacena en `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Cuando se inicia OpenAiCode, carga los proveedores desde el archivo de credenciales. Y si hay claves definidas en sus entornos o un archivo `.env` en su proyecto.
...
Utilice el indicador `--refresh` para actualizar la lista de modelos almacenados en caché. Esto es útil cuando se han agregado nuevos modelos a un proveedor y desea verlos en OpenAiCode.
...
Inicie un servidor OpenAiCode sin cabeza para acceso API. Consulte los [documentos del servidor](/docs/server) para conocer la interfaz HTTP completa.
...
Administrar OpenAiCode sesiones.
...
Enumere todas las sesiones OpenAiCode.
...
Muestre el uso de tokens y las estadísticas de costos para sus sesiones OpenAiCode.
...
Importe datos de sesión desde un archivo JSON o una URL compartida OpenAiCode.

```bash
opencode import <file>
```

Puede importar desde un archivo local o una URL compartida OpenAiCode.
...
Inicie un servidor OpenAiCode sin cabeza con una interfaz web.

```bash
opencode web
```

Esto inicia un servidor HTTP y abre un navegador web para acceder a OpenAiCode a través de una interfaz web. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `opencode`).
...
Desinstale OpenAiCode y elimine todos los archivos relacionados.
...
La CLI de OpenAiCode toma las siguientes banderas globales.
...
OpenAiCode se puede configurar mediante variables de entorno.
