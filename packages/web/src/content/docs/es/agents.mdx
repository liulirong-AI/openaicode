...
Hay dos tipos de agentes en OpenAiCode; agentes primarios y subagentes.
...
OpenAiCode viene con dos agentes principales integrados, **Build** y **Plan**. Bien
...
OpenAiCode viene con dos subagentes integrados, **General** y **Explore**. Veremos esto a continuación.

---

## Integrados

OpenAiCode viene con dos agentes primarios integrados y dos subagentes integrados.
...
Si no se especifica ninguna temperatura, OpenAiCode utiliza valores predeterminados específicos del modelo; normalmente 0 para la mayoría de los modelos, 0,55 para los modelos Qwen.
...
Esta ruta es relativa a donde se encuentra el archivo de configuración. Entonces esto funciona tanto para la configuración global OpenAiCode como para la configuración específica del proyecto.
...
El ID del modelo en su configuración OpenAiCode usa el formato `provider/model-id`. Por ejemplo, si usa [OpenAiCode Zen](/docs/zen), usaría `opencode/gpt-5.1-codex` para GPT 5.1 Codex.
