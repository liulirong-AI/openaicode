...
OpenAiCode Go es una suscripción de bajo coste de **10 $/mes** que te ofrece acceso fiable a modelos populares de código abierto.

:::note
OpenAiCode Go está actualmente en beta.
:::

Go funciona como cualquier otro proveedor en OpenAiCode. Te suscribes a OpenAiCode Go y obtienes tu clave API. Es **completamente opcional** y no necesitas usarlo para utilizar OpenAiCode.
...
:::tip
Hemos probado un grupo selecto de modelos y proveedores que funcionan bien con OpenAiCode.
:::
...
OpenAiCode Go te da acceso a estos modelos por **10 $/mes**.

---

## Cómo funciona

OpenAiCode Go funciona como cualquier otro proveedor en OpenAiCode.

1. Inicias sesión en **<a href={console}>OpenAiCode Zen</a>**, te suscribes a Go y copias tu clave API.
2. Ejecutas el comando `/connect` en la TUI, seleccionas `OpenAiCode Go` y pegas tu clave API.
3. Ejecuta `/models` en la TUI para ver la lista de modelos disponibles a través de Go.

:::note
Solo un miembro por espacio de trabajo puede suscribirse a OpenAiCode Go.
:::
...
OpenAiCode Go incluye los siguientes límites:
...
OpenAiCode Go es un plan de suscripción de **10 $/mes**. A continuación se muestran los precios **por 1M de tokens**.
...
El [model id](/docs/config/#models) en tu configuración de OpenAiCode usa el formato `opencode-go/<model-id>`. Por ejemplo, para Kimi K2.5, usarías `opencode-go/kimi-k2.5` en tu configuración.
...
4. **Sin ataduras**, permitiéndote usar cualquier otro proveedor con OpenAiCode también.
