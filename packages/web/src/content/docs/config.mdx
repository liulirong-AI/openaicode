---
title: Config
description: Using the OpenAiCode JSON config.
---

You can configure OpenAiCode using a JSON config file.

---

## Format

OpenAiCode supports both **JSON** and **JSONC** (JSON with Comments) formats.
...
Place your global OpenAiCode config in `~/.config/opencode/opencode.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.
...
When OpenAiCode starts up, it looks for a config file in the current directory or traverse up to the nearest Git directory.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your OpenAiCode server.
...
You can configure the providers and models you want to use in your OpenAiCode config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, OpenAiCode tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, OpenAiCode will fall back to `"build"` with a warning.
...
OpenAiCode will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
[Plugins](/docs/plugins) extend OpenAiCode with custom tools, hooks, and integrations.
...
This is useful when you want to restrict OpenAiCode to only use specific providers rather than disabling them one by one.
