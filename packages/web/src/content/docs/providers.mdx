---
title: Providers
description: Using any LLM provider in OpenAiCode.
---

import config from "../../../config.mjs"
export const console = config.console

OpenAiCode uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your OpenAiCode config.
...
You can customize the providers through the `provider` section in your OpenAiCode
config.
...
OpenAiCode Zen is a list of models provided by the OpenAiCode team that have been
tested and verified to work well with OpenAiCode. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with OpenAiCode Zen.
:::

1. Run the `/connect` command in the TUI, select `OpenAiCode Zen`, and head to [opencode.ai/auth](https://opencode.ai/zen).
...
It works like any other provider in OpenAiCode and is completely optional to use.

---

## OpenAiCode Go

OpenAiCode Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the OpenAiCode team that have been
tested and verified to work well with OpenAiCode.

1. Run the `/connect` command in the TUI, select `OpenAiCode Go`, and head to [opencode.ai/auth](https://opencode.ai/zen).
...
To use Amazon Bedrock with OpenAiCode:
...
:::info
Using your Claude Pro/Max subscription in OpenAiCode is not officially supported by [Anthropic](https://anthropic.com).
:::
...
OpenAiCode uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock OpenAiCode
...
To use Google Vertex AI with OpenAiCode:
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each OpenAiCode conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each OpenAiCode conversation listed as a separate session.
...
:::tip
Ollama can automatically configure itself for OpenAiCode. See the [Ollama integration docs](https://docs.ollama.com/integrations/opencode) for details.
:::
...
To use Ollama Cloud with OpenAiCode:
...
3. Copy the API key for use in OpenAiCode.
...
6. **Important**: Before using cloud models in OpenAiCode, you must pull the model information locally:
...
OpenAiCode Zen is a list of tested and verified models provided by the OpenAiCode team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>OpenAiCode Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **OpenAiCode Zen**.

   ```txt
   /connect
   ```

3. Enter your OpenAiCode API key.
...
The `limit` fields allow OpenAiCode to understand how much context you have left. Standard providers pull these from models.dev automatically.
