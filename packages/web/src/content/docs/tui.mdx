---
title: TUI
description: Using the OpenAiCode terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenAiCode provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running OpenAiCode starts the TUI for the current directory.
...
When using the OpenAiCode TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to OpenAiCode. Allows you to select from available providers and add their API keys.
...
Exit OpenAiCode. _Aliases_: `/quit`, `/q`
