...
W OpenAiCode obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
OpenAiCode zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
OpenAiCode ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

OpenAiCode ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Jeśli nie zostanie skonfigurowana, OpenAiCode używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji OpenAiCode, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji OpenAiCode w `provider/model-id`. Na przykład, [OpenAiCode Zen](/docs/zen), użyj `opencode/gpt-5.1-codex` dla GPT 5.1 Codex.
