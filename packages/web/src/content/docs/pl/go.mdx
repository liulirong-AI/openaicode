...
OpenAiCode Go to tania subskrypcja za **10 USD miesięcznie**, która zapewnia niezawodny dostęp do popularnych otwartych modeli kodowania.

:::note
OpenAiCode Go jest obecnie w fazie beta.
:::

Go działa jak każdy inny dostawca w OpenAiCode. Subskrybujesz OpenAiCode Go i otrzymujesz swój klucz API. Jest to **całkowicie opcjonalne** i nie musisz z tego korzystać, aby używać OpenAiCode.
...
:::tip
Przetestowaliśmy wybraną grupę modeli i dostawców, którzy dobrze współpracują z OpenAiCode.
:::
...
OpenAiCode Go daje dostęp do tych modeli za **10 USD miesięcznie**.

---

## Jak to działa

OpenAiCode Go działa jak każdy inny dostawca w OpenAiCode.

1. Logujesz się do **<a href={console}>OpenAiCode Zen</a>**, subskrybujesz Go i kopiujesz swój klucz API.
2. Uruchamiasz polecenie `/connect` w TUI, wybierasz `OpenAiCode Go` i wklejasz swój klucz API.
3. Uruchom `/models` w TUI, aby zobaczyć listę modeli dostępnych przez Go.

:::note
Tylko jeden członek na obszar roboczy może subskrybować OpenAiCode Go.
:::
...
OpenAiCode Go obejmuje następujące limity:
...
OpenAiCode Go to plan subskrypcji za **10 USD miesięcznie**. Poniżej znajdują się ceny **za 1 mln tokenów**.
...
[Identyfikator modelu](/docs/config/#models) w twojej konfiguracji OpenAiCode używa formatu `opencode-go/<model-id>`. Na przykład dla Kimi K2.5 użyłbyś `opencode-go/kimi-k2.5` w swojej konfiguracji.
...
Stworzyliśmy OpenAiCode Go, aby:
...
4. Nie wprowadzać **żadnych blokad (lock-in)**, pozwalając na korzystanie z dowolnego innego dostawcy w OpenAiCode.
