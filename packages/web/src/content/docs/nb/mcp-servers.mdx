...
Du kan legge til eksterne verktøy i OpenAiCode ved å bruke _Model Context Protocol_ eller MCP. OpenAiCode støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [OpenAiCode Config](https://opencode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.
...
OpenAiCode håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil OpenAiCode:
...
Hvis serveren krever autentisering, vil OpenAiCode be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `opencode mcp auth <server-name>`.
...
Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil OpenAiCode lagre tokens sikkert i `~/.local/share/opencode/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i OpenAiCode, sammen med innebygde verktøy. Så du kan administrere dem gjennom OpenAiCode-konfigurasjonen som alle andre verktøy.
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble OpenAiCode til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver OpenAiCode-samtale oppført som en separat økt.
