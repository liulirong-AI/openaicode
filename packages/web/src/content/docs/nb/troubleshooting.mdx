...
For å feilsøke problemer med OpenAiCode, start med å sjekke loggene og lokale data den lagrer på disken.
...
OpenAiCode lagrer øktdata og andre applikasjonsdata på disken på:
...
OpenAiCode Desktop kjører en lokal OpenAiCode-server (`opencode-cli` sidevognen) i bakgrunnen. De fleste problemene er forårsaket av en plugin som fungerer dårlig, en ødelagt cache eller en dårlig serverinnstilling.

### Raske sjekker

- Avslutt og start appen på nytt.
- Hvis appen viser en feilskjerm, klikker du på **Start på nytt** og kopierer feildetaljene.
- Bare macOS: `OpenAiCode`-meny -> **Last nettvisning på nytt** (hjelper hvis UI er tom/frosset).
...
OpenAiCode kan også laste lokale plugins fra disken. Flytt disse midlertidig ut av veien (eller gi nytt navn til mappen) og start skrivebordsappen på nytt:
...
Hvis deaktivering av plugins ikke hjelper (eller en plugin-installasjon sitter fast), tøm hurtigbufferen slik at OpenAiCode kan gjenoppbygge den.

1. Avslutt OpenAiCode Desktop helt.
...
3. Start OpenAiCode Desktop på nytt.

---

### Løs problemer med servertilkobling

OpenAiCode Desktop kan enten starte sin egen lokale server (standard) eller koble til en server URL du har konfigurert.
...
På Windows krever OpenAiCode Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åpnes i et tomt vindu eller ikke starter, installer/oppdater WebView2 og prøv igjen.

---

### Windows: Generelle ytelsesproblemer

Hvis du opplever treg ytelse, problemer med filtilgang eller terminalproblemer på Windows, kan du prøve å bruke [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL gir et Linux-miljø som fungerer mer sømløst med funksjonene til OpenAiCode.

---

### Varsler vises ikke

OpenAiCode Desktop viser bare systemvarsler når:

- varsler er aktivert for OpenAiCode i OS-innstillingene dine, og
...
1. Avslutt OpenAiCode Desktop.
2. Finn og slett disse filene (de finnes i OpenAiCode Desktop-appens datakatalog):
...
Hvis du støter på API-anropsfeil, kan dette skyldes utdaterte provider-pakker. OpenAiCode installerer dynamisk provider-pakker (OpenAI, Anthropic, Google, etc.) etter behov og cacher dem lokalt.
...
2. Start OpenAiCode på nytt for å installere de nyeste provider-pakkene på nytt

Dette vil tvinge OpenAiCode til å laste ned de nyeste versjonene av provider-pakkene, som ofte løser kompatibilitetsproblemer med modellparametere og API-endringer.
...
OpenAiCode vil oppdage om du bruker Wayland og foretrekker `wl-clipboard`, ellers vil den prøve å finne utklippstavle-verktøy i rekkefølgen: `xclip` og `xsel`.
