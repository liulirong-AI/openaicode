...
description: Skriv dine egne programtillegg for å utvide OpenAiCode.
---

Plugins lar deg utvide OpenAiCode ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til OpenAiCode.
...
OpenAiCode kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En OpenAiCode SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide OpenAiCode.
...
:::note
Hvis du bruker OpenAiCode-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre OpenAiCode fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til OpenAiCode:
...
`tool`-hjelperen lager et tilpasset verktøy som OpenAiCode kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for OpenAiCode sammen med innebygde verktøy.
