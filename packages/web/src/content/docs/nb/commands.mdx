...
Du kan legge til egendefinerte kommandoer gjennom OpenAiCode-konfigurasjonen eller ved å lage markdown-filer i katalogen `commands/`.

---

### JSON

Bruk alternativet `command` i OpenAiCode [config](/docs/config):
...
OpenAiCode inkluderer flere innebygde kommandoer som `/init`, `/undo`, `/redo`, `/share`, `/help`; [finn ut mer](/docs/tui#commands).
