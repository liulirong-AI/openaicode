...
Du kan konfigurere OpenAiCode ved å bruke en JSON konfigurasjonsfil.

---

## Format

OpenAiCode støtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
Plasser din globale OpenAiCode-konfigurasjon i `~/.config/opencode/opencode.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Når OpenAiCode starter opp, ser den etter en konfigurasjonsfil i gjeldende katalog eller går opp til nærmeste Git-katalog.
...
- `mdns` - Aktiver mDNS-tjenesteoppdagelse. Dette lar andre enheter på nettverket oppdage OpenAiCode-serveren din.
...
Du kan konfigurere leverandørene og modellene du vil bruke i OpenAiCode-konfigurasjonen gjennom alternativene `provider`, `model` og `small_model`.
...
Alternativet `small_model` konfigurerer en egen modell for lette oppgaver som tittelgenerering. Som standard prøver OpenAiCode å bruke en rimeligere modell hvis en er tilgjengelig fra leverandøren din, ellers faller den tilbake til hovedmodellen din.
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](/docs/agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil OpenAiCode falle tilbake til `"build"` med en advarsel.
...
OpenAiCode vil automatisk laste ned eventuelle nye oppdateringer når den starter opp. Du kan deaktivere dette med alternativet `autoupdate`.
...
Som standard **tillater OpenAiCode alle operasjoner** uten å kreve eksplisitt godkjenning. Du kan endre dette ved å bruke alternativet `permission`.
...
[Plugins](/docs/plugins) utvider OpenAiCode med tilpassede verktøy, kroker og integrasjoner.
...
Dette er nyttig når du vil begrense OpenAiCode til kun å bruke spesifikke leverandører i stedet for å deaktivere dem én etter én.
