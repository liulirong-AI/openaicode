...
Det er to typer agenter i OpenAiCode; primære agenter og underagenter.
...
OpenAiCode kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
OpenAiCode kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

OpenAiCode kommer med to innebygde primære agenter og to innebygde underagenter.
...
Hvis ingen temperatur er spesifisert, bruker OpenAiCode modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale OpenAiCode-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i OpenAiCode-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [OpenAiCode Zen](/docs/zen), vil du bruke `opencode/gpt-5.1-codex` for GPT 5.1 Codex.
