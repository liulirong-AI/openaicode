---
title: Regler
description: Set custom instructions for OpenAiCode.
---

Du kan gi tilpassede instruksjoner for å OpenAiCode ved å opprette en `AGENTS.md`-fil. Dette ligner på Cursors regler. Den inneholder instruksjoner som vil bli inkludert i LLMs kontekst for å tilpasse oppførselen til ditt spesifikke prosjekt.

---

## Initialisering

For å lage en ny `AGENTS.md` fil, kan du kjøre kommandoen `/init` i OpenAiCode.

:::tip
Du bør sjekke inn prosjektets `AGENTS.md`-fil i Git.
:::

Dette vil skanne prosjektet og alt dets innhold for å forstå hva prosjektet handler om og generere en `AGENTS.md`-fil med det. Dette hjelper OpenAiCode å navigere i prosjektet bedre.
...
OpenAiCode støtter også lesing av `AGENTS.md`-filen fra flere steder. Og dette tjener forskjellige formål.
...
Du kan også ha globale regler i en `~/.config/opencode/AGENTS.md`-fil. Dette blir brukt på alle OpenAiCode-økter.
...
For brukere som migrerer fra Claude Code, støtter OpenAiCode Claude Codes filkonvensjoner som fallbacks:
...
Når OpenAiCode starter, ser den etter regelfiler i denne rekkefølgen:
...
Selv om OpenAiCode ikke automatisk analyserer filreferanser i `AGENTS.md`, kan du oppnå lignende funksjonalitet på to måter:
...
Du kan lære OpenAiCode å lese eksterne filer ved å gi eksplisitte instruksjoner i `AGENTS.md`. Her er et praktisk eksempel:
...
- Sørge for at OpenAiCode laster filer kun når det er nødvendig for den spesifikke oppgaven
