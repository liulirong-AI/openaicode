...
:::caution
Moduser konfigureres nå gjennom alternativet `agent` i OpenAiCode-konfigurasjonen.
Alternativet `mode` er nå utdatert. [Finn ut mer](/docs/agents).
:::

Moduser i OpenAiCode lar deg tilpasse oppførselen, verktøyene og prompter for ulike brukstilfeller.

Den kommer med to innebygde moduser: **bygg** og **plan**. Du kan tilpasse
disse eller konfigurer din egen gjennom OpenAiCode-konfigurasjonen.
...
OpenAiCode kommer med to innebygde moduser.
...
Hvis ingen temperatur er spesifisert, bruker OpenAiCode modellspesifikke standardinnstillinger (vanligvis 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
Denne banen er relativ til der konfigurasjonsfilen er plassert. Så dette fungerer for både den globale OpenAiCode-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
