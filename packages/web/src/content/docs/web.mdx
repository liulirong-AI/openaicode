---
title: Web
description: Using OpenAiCode in your browser.
---

OpenAiCode can run as a web application in your browser, providing the same powerful AI coding experience without needing a terminal.

![OpenAiCode Web - New Session](../../assets/web/web-homepage-new-session.png)
...
This starts a local server on `127.0.0.1` with a random available port and automatically opens OpenAiCode in your default browser.
...
By default, OpenAiCode picks an available port. You can specify a port:
...
By default, the server binds to `127.0.0.1` (localhost only). To make OpenAiCode accessible on your network:

```bash
opencode web --hostname 0.0.0.0
```

When using `0.0.0.0`, OpenAiCode will display both local and network addresses:
...
Once started, the web interface provides access to your OpenAiCode sessions.
