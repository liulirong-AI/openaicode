...
Puoi aggiungere strumenti esterni a OpenAiCode usando il _Model Context Protocol_, o MCP. OpenAiCode supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [OpenAiCode Config](https://opencode.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
OpenAiCode gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, OpenAiCode:
...
Se il server richiede autenticazione, OpenAiCode ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `opencode mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, OpenAiCode memorizzera' i token in modo sicuro in `~/.local/share/opencode/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in OpenAiCode insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione OpenAiCode come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare OpenAiCode al tuo account Sentry.
