---
title: Zen
description: Lista curata di modelli fornita da OpenAiCode.
---
...
OpenAiCode Zen è una lista di modelli testati e verificati dal team di OpenAiCode.

:::note
OpenAiCode Zen è attualmente in beta.
:::

Zen funziona come qualunque altro provider in OpenAiCode. Accedi a OpenAiCode Zen e ottieni la tua chiave API. È **completamente opzionale**: non devi usarlo per usare OpenAiCode.
...
:::tip
Abbiamo testato un gruppo selezionato di modelli e provider che funzionano bene con OpenAiCode.
:::
...
OpenAiCode Zen e un gateway AI che ti dà accesso a questi modelli.

---

## Come funziona

OpenAiCode Zen funziona come qualunque altro provider in OpenAiCode.

1. Accedi a **<a href={console}>OpenAiCode Zen</a>**, aggiungi i dettagli di fatturazione e copia la chiave API.
2. Esegui il comando `/connect` nella TUI, seleziona OpenAiCode Zen e incolla la chiave API.
...
Il [model id](/docs/config/#models) nella config di OpenAiCode usa il formato `opencode/<model-id>`. Per esempio, per GPT 5.2 Codex useresti `opencode/gpt-5.2-codex` nella config.
...
- MiniMax M2.5 Free e disponibile su OpenAiCode per un periodo limitato. Il team usa questo tempo per raccogliere feedback e migliorare il modello.
- Big Pickle è un modello stealth gratuito su OpenAiCode per un periodo limitato. Il team usa questo tempo per raccogliere feedback e migliorare il modello.
...
Abbiamo creato OpenAiCode Zen per:
...
4. Evitare **lock-in** permettendoti di usarlo con qualunque altro agente di coding e lasciandoti sempre usare anche altri provider con OpenAiCode.
