---
title: Provider
description: Usa qualsiasi provider LLM in OpenAiCode.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenAiCode usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di OpenAiCode.
...
OpenAiCode Zen e una lista di modelli forniti dal team OpenAiCode, testati e
verificati per funzionare bene con OpenAiCode. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da OpenAiCode Zen.
:::
...
Funziona come qualsiasi altro provider in OpenAiCode ed e totalmente opzionale.
...
Per usare Amazon Bedrock con OpenAiCode:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in OpenAiCode non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
OpenAiCode usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare OpenAiCode
...
Per usare Google Vertex AI con OpenAiCode:
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) per loggare automaticamente ogni conversazione di OpenAiCode come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di OpenAiCode elencata come sessione separata.
...
:::tip
Ollama può auto-configurarsi per OpenAiCode. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/opencode) per dettagli.
:::
...
Per usare Ollama Cloud con OpenAiCode:
...
3. Copia la chiave API da usare in OpenAiCode.
...
6. **Importante**: Prima di usare modelli cloud in OpenAiCode, devi scaricare le informazioni del modello localmente:
...
OpenAiCode Zen è una lista di modelli testati e verificati forniti dal team OpenAiCode. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>OpenAiCode Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **OpenAiCode Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di OpenAiCode.
...
I campi `limit` permettono a OpenAiCode di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
