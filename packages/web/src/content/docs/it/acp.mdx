---
title: Supporto ACP
description: Usa OpenAiCode in qualunque editor compatibile con ACP.
---

OpenAiCode supporta [Agent Client Protocol](https://agentclientprotocol.com) (ACP), permettendoti di usarlo direttamente in editor e IDE compatibili.
...
Per usare OpenAiCode via ACP, configura l'editor in modo che esegua il comando `opencode acp`.

Il comando avvia OpenAiCode come sottoprocesso compatibile con ACP che comunica con l'editor via JSON-RPC su stdio.
...
Per aprirlo, seleziona il nuovo agente 'OpenAiCode' nel selettore agenti di AI Chat.
...
Per usare OpenAiCode come agente ACP in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), aggiungi quanto segue alla config di Neovim:
...
Questa config imposta CodeCompanion per usare OpenAiCode come agente ACP per la chat.
...
OpenAiCode funziona via ACP come nel terminale. Tutte le funzionalita sono supportate:
:::note
...
- Server MCP configurati nella tua configurazione OpenAiCode
