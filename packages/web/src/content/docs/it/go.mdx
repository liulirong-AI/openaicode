...
OpenAiCode Go è un abbonamento a basso costo di **$10/mese** che ti offre un accesso affidabile ai modelli di coding open source più popolari.

:::note
OpenAiCode Go è attualmente in beta.
:::

Go funziona come qualsiasi altro provider in OpenAiCode. Ti abboni a OpenAiCode Go e ottieni la tua chiave API. È **completamente opzionale** e non è necessario utilizzarlo per usare OpenAiCode.
...
OpenAiCode Go ti dà accesso a questi modelli per **$10/mese**.

---

## Come funziona

OpenAiCode Go funziona come qualsiasi altro provider in OpenAiCode.

1. Accedi a **<a href={console}>OpenAiCode Zen</a>**, abbonati a Go e copia la tua chiave API.
2. Esegui il comando `/connect` nella TUI, seleziona `OpenAiCode Go` e incolla la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a OpenAiCode Go.
:::
...
OpenAiCode Go include i seguenti limiti:
...
OpenAiCode Go è un piano di abbonamento da **$10/mese**. Di seguito sono riportati i prezzi **per 1M di token**.
...
Il [model id](/docs/config/#models) nella tua configurazione OpenAiCode utilizza il formato `opencode-go/<model-id>`. Ad esempio, per Kimi K2.5, useresti `opencode-go/kimi-k2.5` nella tua configurazione.
...
Abbiamo creato OpenAiCode Go per:
...
4. Non avere **alcun lock-in** permettendoti di utilizzare qualsiasi altro provider con OpenAiCode.
