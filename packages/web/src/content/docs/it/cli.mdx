---
title: CLI
description: Opzioni e comandi della CLI di OpenAiCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI di OpenAiCode, per impostazione predefinita, avvia la [TUI](/docs/tui) quando viene eseguita senza argomenti.

```bash
opencode
```

Ma accetta anche i comandi documentati in questa pagina. Questo ti permette di interagire con OpenAiCode in modo programmatico.
...
Avvia l'interfaccia testuale di OpenAiCode (interfaccia utente da terminale).
...
La CLI di OpenAiCode include anche i seguenti comandi.

---

### agent

Gestisci gli agenti per OpenAiCode.
...
Collega un terminale a un backend server di OpenAiCode già in esecuzione avviato tramite i comandi `serve` o `web`.

```bash
opencode attach [url]
```

Questo consente di usare la TUI con un backend OpenAiCode remoto. Per esempio:
...
OpenAiCode si basa sull'elenco provider di [Models.dev](https://models.dev), quindi puoi usare `opencode auth login` per configurare le API key per qualunque provider tu voglia usare. Le credenziali vengono salvate in `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Quando OpenAiCode si avvia, carica i provider dal file delle credenziali e, se presenti, anche eventuali key definite nell'ambiente o in un file `.env` nel progetto.
...
Usa `--refresh` per aggiornare l'elenco modelli in cache. È utile quando nuovi modelli vengono aggiunti a un provider e vuoi vederli in OpenAiCode.
...
Avvia un server OpenAiCode headless per accesso via API. Vedi le [server docs](/docs/server) per l'interfaccia HTTP completa.
...
Gestisci le sessioni OpenAiCode.
...
Elenca tutte le sessioni OpenAiCode.
...
Mostra statistiche di utilizzo token e costo per le sessioni OpenAiCode.
...
Importa i dati di sessione da un file JSON o da un URL di condivisione OpenAiCode.

```bash
opencode import <file>
```

Puoi importare da un file locale o da un URL di condivisione OpenAiCode.
...
Avvia un server OpenAiCode headless con interfaccia web.

```bash
opencode web
```

Avvia un server HTTP e apre un browser per accedere a OpenAiCode tramite interfaccia web. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `opencode`).
...
Disinstalla OpenAiCode e rimuove tutti i file correlati.
...
OpenAiCode può essere configurato tramite variabili d'ambiente.
