...
description: Usa la configurazione JSON di OpenAiCode.
---

Puoi configurare OpenAiCode usando un file di configurazione JSON.

---

## Formato

OpenAiCode supporta sia **JSON** sia **JSONC** (JSON con commenti).
...
Metti la configurazione globale di OpenAiCode in `~/.config/opencode/opencode.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.
...
Quando OpenAiCode si avvia, cerca un file di configurazione nella directory corrente oppure risale fino alla directory Git piu vicina.
...
- `mdns` - Abilita la scoperta del servizio mDNS. Questo permette ad altri dispositivi sulla rete di scoprire il tuo server OpenAiCode.
...
Puoi configurare provider e modelli da usare in OpenAiCode tramite le opzioni `provider`, `model` e `small_model`.
...
L'opzione `small_model` configura un modello separato per task leggeri come la generazione dei titoli. Per impostazione predefinita, OpenAiCode prova a usare un modello più economico se disponibile dal tuo provider, altrimenti fa fallback sul tuo modello principale.
...
L'agente predefinito deve essere un agente primary (non un subagent). Puo essere un agente integrato come `"build"` o `"plan"`, oppure un [agente personalizzato](/docs/agents) che hai definito. Se l'agente specificato non esiste o e un subagent, OpenAiCode fara fallback su `"build"` mostrando un avviso.
...
OpenAiCode scarichera automaticamente eventuali aggiornamenti quando si avvia. Puoi disabilitare questa funzione con l'opzione `autoupdate`.
...
[I plugin](/docs/plugins) estendono OpenAiCode con strumenti personalizzati, hook e integrazioni.
...
E utile quando vuoi limitare OpenAiCode a usare solo alcuni provider invece di disabilitarli uno per uno.
