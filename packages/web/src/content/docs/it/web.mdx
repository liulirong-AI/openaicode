---
title: Web
description: Usa OpenAiCode nel browser.
---

OpenAiCode puo funzionare come applicazione web nel browser, offrendo la stessa potente esperienza di coding AI senza bisogno di un terminale.

![OpenAiCode Web - Nuova sessione](../../../assets/web/web-homepage-new-session.png)
...
Questo avvia un server locale su `127.0.0.1` con una porta disponibile scelta a caso e apre automaticamente OpenAiCode nel browser predefinito.
...
Per impostazione predefinita, OpenAiCode sceglie una porta disponibile. Puoi specificare una porta:
...
Per impostazione predefinita, il server si lega a `127.0.0.1` (solo localhost). Per rendere OpenAiCode accessibile sulla rete:

```bash
opencode web --hostname 0.0.0.0
```

Usando `0.0.0.0`, OpenAiCode mostrera sia l'indirizzo locale sia quello di rete:
...
Una volta avviata, l'interfaccia web ti da accesso alle tue sessioni OpenAiCode.

### Sessioni

Visualizza e gestisci le sessioni dalla homepage. Puoi vedere le sessioni attive e avviarne di nuove.

![OpenAiCode Web - Sessione attiva](../../../assets/web/web-homepage-active-session.png)

### Stato del server

Fai clic su "See Servers" per vedere i server connessi e il loro stato.

![OpenAiCode Web - See Servers](../../../assets/web/web-homepage-see-servers.png)
