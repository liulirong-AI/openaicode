---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di OpenAiCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenAiCode fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire OpenAiCode avvia la TUI per la directory corrente.
...
Quando usi la TUI di OpenAiCode, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a OpenAiCode. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da OpenAiCode. _Alias_: `/quit`, `/q`
