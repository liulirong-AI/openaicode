...
In OpenAiCode esistono due tipi di agenti: agenti primari e subagenti.
...
OpenAiCode include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
OpenAiCode include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

OpenAiCode include due agenti primari integrati e due subagenti integrati.
...
Se non specifichi una temperature, OpenAiCode usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di OpenAiCode sia per la config specifica del progetto.
...
L'ID modello nella configurazione OpenAiCode usa il formato `provider/model-id`. Per esempio, se stai usando [OpenAiCode Zen](/docs/zen), useresti `opencode/gpt-5.1-codex` per GPT 5.1 Codex.
