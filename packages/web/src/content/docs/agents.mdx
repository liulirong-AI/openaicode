...
There are two types of agents in OpenAiCode; primary agents and subagents.
...
OpenAiCode comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
OpenAiCode comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

OpenAiCode comes with two built-in primary agents and two built-in subagents.
...
If no temperature is specified, OpenAiCode uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global OpenAiCode config and the project specific config.
...
The model ID in your OpenAiCode config uses the format `provider/model-id`. For example, if you're using [OpenAiCode Zen](/docs/zen), you would use `opencode/gpt-5.1-codex` for GPT 5.1 Codex.
