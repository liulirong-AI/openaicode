...
description: Verwenden Sie OpenAiCode in jedem ACP-kompatiblen Editor.
---

OpenAiCode unterstützt [Agent Client Protocol](https://agentclientprotocol.com) oder (ACP), sodass Sie es direkt in kompatiblen Editoren und IDEs verwenden können.
...
Um OpenAiCode über ACP zu verwenden, konfigurieren Sie Ihren Editor für die Ausführung des Befehls `opencode acp`.

Der Befehl startet OpenAiCode als ACP-kompatiblen Unterprozess, der über stdio über JSON-RPC mit Ihrem Editor kommuniziert.
...
Um es zu öffnen, verwenden Sie den neuen Agenten „OpenAiCode“ in der AI Chat Agent Selector.
...
Um OpenAiCode als ACP-Agent in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim) zu verwenden, fügen Sie Folgendes zu Ihrer Neovim-Konfiguration hinzu:
...
Diese Konfiguration richtet CodeCompanion so ein, dass OpenAiCode als ACP-Agent für den Chat verwendet wird.
...
OpenAiCode funktioniert über ACP genauso wie im Terminal. Alle Funktionen werden unterstützt:
...
- MCP-Server, die in Ihrer OpenAiCode-Konfiguration konfiguriert sind
