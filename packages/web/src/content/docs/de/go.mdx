...
OpenAiCode Go ist ein kostengünstiges Abonnement für **10 $/Monat**, das dir zuverlässigen Zugriff auf beliebte Open-Coding-Modelle bietet.

:::note
OpenAiCode Go befindet sich derzeit in der Beta-Phase.
:::

Go funktioniert wie jeder andere Anbieter in OpenAiCode. Du abonnierst OpenAiCode Go und erhältst deinen API-Schlüssel. Es ist **völlig optional** und du musst es nicht nutzen, um OpenAiCode zu verwenden.
...
:::tip
Wir haben eine ausgewählte Gruppe von Modellen und Anbietern getestet, die gut mit OpenAiCode funktionieren.
:::
...
OpenAiCode Go gibt dir Zugriff auf diese Modelle für **10 $/Monat**.

---

## Wie es funktioniert

OpenAiCode Go funktioniert wie jeder andere Anbieter in OpenAiCode.

1. Du meldest dich bei **<a href={console}>OpenAiCode Zen</a>** an, abonnierst Go und kopierst deinen API-Schlüssel.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `OpenAiCode Go` und fügst deinen API-Schlüssel ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann OpenAiCode Go abonnieren.
:::
...
OpenAiCode Go beinhaltet die folgenden Limits:
...
OpenAiCode Go ist ein Abonnementplan für **10 $/Monat**. Unten stehen die Preise **pro 1 Mio. Token**.
...
Die [Modell-ID](/docs/config/#models) in deiner OpenAiCode-Konfiguration verwendet das Format `opencode-go/<model-id>`. Zum Beispiel würdest du für Kimi K2.5 `opencode-go/kimi-k2.5` in deiner Konfiguration verwenden.
...
Wir haben OpenAiCode Go erstellt, um:
...
4. **Keinen Lock-in** zu haben, indem wir dir ermöglichen, jeden anderen Anbieter ebenfalls mit OpenAiCode zu nutzen.
