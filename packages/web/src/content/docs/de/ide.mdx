---
title: IDE
description: Die OpenAiCode-Erweiterung für VS Code, Cursor und andere IDEs
---

OpenAiCode lässt sich in VS Code, Cursor oder jede IDE integrieren, die ein Terminal unterstützt. Führen Sie einfach `opencode` im Terminal aus, um zu beginnen.

---

## Verwendung

- **Schnellstart**: Verwenden Sie `Cmd+Esc` (Mac) oder `Ctrl+Esc` (Windows/Linux), um OpenAiCode in einer geteilten Terminalansicht zu öffnen, oder fokussieren Sie eine dieser Terminalsitzung, falls bereits eine ausgeführt wird.
- **Neue Sitzung**: Verwenden Sie `Cmd+Shift+Esc` (Mac) oder `Ctrl+Shift+Esc` (Windows/Linux), um eine neue OpenAiCode-Terminalsitzung zu starten, auch wenn eine bereits geöffnet ist. Sie können auch auf die OpenAiCode-Schaltfläche im UI klicken.
- **Kontextbewusstsein**: Teilen Sie Ihre aktuelle Auswahl oder Registerkarte automatisch mit OpenAiCode.
...
So installieren Sie OpenAiCode in VS Code und beliebten Forks wie Cursor, Windsurf, VSCodium:
...
Suchen Sie im Extension Marketplace nach **OpenAiCode** und klicken Sie auf **Installieren**.
