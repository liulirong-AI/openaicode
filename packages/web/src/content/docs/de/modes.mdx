...
:::caution
Modi werden jetzt über die Option `agent` in der OpenAiCode-Konfiguration konfiguriert. Der
Die Option `mode` ist jetzt veraltet. [Learn more](/docs/agents).
:::

Mit den Modi in OpenAiCode können Sie das Verhalten, die Tools und die Eingabeaufforderungen für verschiedene Anwendungsfälle anpassen.

Es verfügt über zwei integrierte Modi: **Build** und **Plan**. Sie können anpassen
diese oder konfigurieren Sie Ihre eigenen über die OpenAiCode-Konfiguration.
...
OpenAiCode verfügt über zwei integrierte Modi.
...
Wenn keine Temperatur angegeben ist, verwendet OpenAiCode modellspezifische Standardwerte (normalerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle).
...
sowohl die globale OpenAiCode-Konfiguration als auch die projektspezifische Konfiguration.
