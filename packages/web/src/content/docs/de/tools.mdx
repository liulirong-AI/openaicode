...
OpenAiCode bringt eingebaute Tools mit und laesst sich ueber [Custom Tools](/docs/custom-tools) oder [MCP-Server](/docs/mcp-servers) erweitern.
...
Hier sind alle in OpenAiCode verfuegbaren eingebauten Tools.
...
:::note
Dieses Tool ist nur verfuegbar, wenn der OpenAiCode-Provider genutzt wird oder die Umgebungsvariable `OPENCODE_ENABLE_EXA` auf einen 'truthy' Wert (z. B. `true` oder `1`) gesetzt ist.

Zum Aktivieren beim Start von OpenAiCode:
