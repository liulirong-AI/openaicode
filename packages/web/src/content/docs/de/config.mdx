---
title: Konfiguration
description: Verwenden Sie die Konfiguration OpenAiCode JSON.
---

Sie können OpenAiCode mithilfe einer JSON-Konfigurationsdatei konfigurieren.

---

## Format

OpenAiCode unterstützt die Formate **JSON** und **JSONC** (JSON mit Kommentaren).
...
Platzieren Sie Ihre globale OpenAiCode-Konfiguration in `~/.config/opencode/opencode.json`. Verwenden Sie die globale Konfiguration für benutzerweite Einstellungen wie Themen, Anbieter oder Tastenkombinationen.
...
Wenn OpenAiCode gestartet wird, sucht es im aktuellen Verzeichnis nach einer Konfigurationsdatei oder durchsucht das nächstgelegene Git-Verzeichnis.
...
- `mdns` – mDNS-Diensterkennung aktivieren. Dadurch können andere Geräte im Netzwerk Ihres OpenAiCode-Servers erkannt werden.
...
Sie können die Anbieter und Modelle, die Sie in Ihrer OpenAiCode-Konfiguration verwenden möchten, über die Optionen `provider`, `model` und `small_model` konfigurieren.
...
Die Option `small_model` konfiguriert ein separates Modell für einfache Aufgaben wie die Titelgenerierung. Standardmäßig versucht OpenAiCode, ein günstigeres Modell zu verwenden, sofern eines bei Ihrem Anbieter verfügbar ist, andernfalls greift es auf Ihr Hauptmodell zurück.
...
Sie können das Thema, das Sie in Ihrer OpenAiCode-Konfiguration verwenden möchten, über die Option `theme` konfigurieren.
...
Der Standardagent muss ein Primäragent (kein Subagent) sein. Dies kann ein integrierter Agent wie `"build"` oder `"plan"` oder ein von Ihnen definierter [custom agent](/docs/agents) sein. Wenn der angegebene Agent nicht existiert oder ein Subagent ist, greift OpenAiCode mit einer Warnung auf `"build"` zurück.
...
OpenAiCode lädt beim Start automatisch alle neuen Updates herunter. Sie können dies mit der Option `autoupdate` deaktivieren.
...
OpenAiCode erlaubt standardmäßig alle Vorgänge, ohne dass eine ausdrückliche Genehmigung erforderlich ist. Sie können dies mit der Option `permission` ändern.
...
[Plugins](/docs/plugins) erweitert OpenAiCode mit benutzerdefinierten Tools, Hooks und Integrationen.
...
Dies ist nützlich, wenn Sie OpenAiCode darauf beschränken möchten, nur bestimmte Anbieter zu verwenden, anstatt sie einzeln zu deaktivieren.
