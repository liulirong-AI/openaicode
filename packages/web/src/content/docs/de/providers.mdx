---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in OpenAiCode.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenAiCode verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer OpenAiCode-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem OpenAiCode anpassen
config.
...
OpenAiCode Zen ist eine Liste von Modellen, die vom OpenAiCode-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit OpenAiCode funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit OpenAiCode Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie OpenAiCode aus und gehen Sie zu [opencode.ai/auth](https://opencode.ai/auth).
...
Es funktioniert wie jeder andere Anbieter in OpenAiCode und ist völlig optional.

---

## OpenAiCode Go

OpenAiCode Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom OpenAiCode-Team getestet und verifiziert wurden, dass sie gut mit OpenAiCode funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `OpenAiCode Go` und gehen Sie zu [opencode.ai/auth](https://opencode.ai/zen).
...
So verwenden Sie Amazon Bedrock mit OpenAiCode:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie OpenAiCode ausführen:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in OpenAiCode wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit OpenAiCode ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre OpenAiCode-Konfiguration hinzufügen.
...
2. Wählen Sie Ihre Authentifizierungsmethode:
   ...
   3. Name: `OpenAiCode`, Bereiche: `api`
...
um das Token im OpenAiCode-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
OpenAiCode verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von OpenAiCode
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit OpenAiCode:
...
So verwenden Sie Google Vertex AI mit OpenAiCode:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie OpenAiCode ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über OpenAiCode konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session)-Plugin, um jede OpenAiCode-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede OpenAiCode-Konversation als separate Sitzung aufgeführt.
...
Sie können OpenAiCode für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können OpenAiCode über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können OpenAiCode über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für OpenAiCode konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/opencode).
:::
...
So verwenden Sie Ollama Cloud mit OpenAiCode:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in OpenAiCode.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in OpenAiCode verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
OpenAiCode Zen ist eine Liste getesteter und verifizierter Modelle, die vom OpenAiCode-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>OpenAiCode Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **OpenAiCode Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren OpenAiCode API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre OpenAiCode-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre OpenAiCode-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit OpenAiCode:
...
Sie können Modelle auch über Ihre OpenAiCode-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit OpenAiCode verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
Mithilfe der `limit`-Felder kann OpenAiCode erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die OpenAiCode-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer OpenAiCode-Konfiguration übereinstimmt.
