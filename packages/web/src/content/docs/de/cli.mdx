---
title: CLI
description: OpenAiCode CLI Optionen und Befehle.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Der OpenAiCode CLI startet standardmäßig das [TUI](/docs/tui), wenn er ohne Argumente ausgeführt wird.

```bash
opencode
```

Es akzeptiert aber auch Befehle, wie auf dieser Seite dokumentiert. Dadurch können Sie programmgesteuert mit OpenAiCode interagieren.
...
Starten Sie die Terminalbenutzeroberfläche OpenAiCode.
...
Der OpenAiCode CLI verfügt außerdem über die folgenden Befehle.

---

### agent

Agenten für OpenAiCode verwalten.
...
Schließen Sie ein Terminal an einen bereits laufenden OpenAiCode-Backend-Server an, der über die Befehle `serve` oder `web` gestartet wurde.

```bash
opencode attach [url]
```

Dies ermöglicht die Verwendung von TUI mit einem Remote-Backend OpenAiCode. Zum Beispiel:
...
OpenAiCode wird von der Anbieterliste bei [Models.dev](https://models.dev) unterstützt, sodass Sie `opencode auth login` verwenden können, um API-Schlüssel für jeden Anbieter zu konfigurieren, den Sie verwenden möchten. Dies wird in `~/.local/share/opencode/auth.json` gespeichert.

```bash
opencode auth login
```

Wenn OpenAiCode startet, lädt es den Anbieter aus der Anmeldeinformationsdatei. Und ob in Ihren Umgebungen Schlüssel sind oder in Ihrem Projekt eine `.env`-Datei vorhanden ist.
...
Verwenden Sie das Flag `--refresh`, um die zwischengespeicherte Modellliste zu aktualisieren. Dies ist nützlich, wenn einem Anbieter neue Modelle hinzugefügt wurden und Sie diese in OpenAiCode sehen möchten.
...
Führen Sie OpenAiCode im nicht interaktiven Modus aus, indem Sie direkt eine Eingabeaufforderung übergeben.
...
| `--attach`   |      | An einen laufenden OpenAiCode-Server anschließen (z.B. http://localhost:4096)                         |
...
Starten Sie einen Headless-OpenAiCode-Server für den API-Zugriff. Sehen Sie sich die [Server-Dokumentation](/docs/server) für die vollständige HTTP-Schnittstelle an.

```bash
opencode serve
```

Dadurch wird ein HTTP-Server gestartet, der API-Zugriff auf OpenAiCode-Funktionalität ohne die TUI-Schnittstelle bietet. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `opencode`).
...
OpenAiCode-Sitzungen verwalten.
...
Listen Sie alle OpenAiCode-Sitzungen auf.
...
Zeigen Sie Token-Nutzungs- und Kostenstatistiken für Ihre OpenAiCode-Sitzungen an.
...
Importieren Sie Sitzungsdaten aus einer JSON-Datei oder einer OpenAiCode-Freigabe-URL.

```bash
opencode import <file>
```

Sie können aus einer lokalen Datei oder einer OpenAiCode-Freigabe-URL importieren.
...
Starten Sie einen Headless-OpenAiCode-Server mit einer Weboberfläche.

```bash
opencode web
```

Dadurch wird ein HTTP-Server gestartet und ein Webbrowser geöffnet, um über eine Webschnittstelle auf OpenAiCode zuzugreifen. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `opencode`).
...
Deinstallieren Sie OpenAiCode und entfernen Sie alle zugehörigen Dateien.
...
Aktualisiert OpenAiCode auf die neueste Version oder eine bestimmte Version.
...
Der OpenAiCode CLI akzeptiert die folgenden globalen Flags.
...
OpenAiCode kann mithilfe von Umgebungsvariablen konfiguriert werden.
