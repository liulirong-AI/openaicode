...
Wenn OpenAiCode Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
OpenAiCode Desktop startet im Hintergrund einen lokalen OpenAiCode-Server (`opencode-cli`-Sidecar).
...
- Nur macOS: `OpenAiCode`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
OpenAiCode kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit OpenAiCode ihn neu aufbauen kann.

1. Beende OpenAiCode Desktop komplett.
...
3. Starte OpenAiCode Desktop neu.

---

### Server-Verbindungsprobleme beheben

OpenAiCode Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Unter Windows benoetigt OpenAiCode Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit OpenAiCode-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

OpenAiCode Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer OpenAiCode in den OS-Einstellungen aktiviert sind, und
...
1. Beende OpenAiCode Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von OpenAiCode Desktop):
...
Wenn du Probleme mit OpenAiCode hast:
