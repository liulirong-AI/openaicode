---
title: Zen
description: Lista selecionada de modelos fornecidos pelo OpenAiCode.
---
...
O OpenAiCode Zen é uma lista de modelos testados e verificados fornecidos pela equipe do OpenAiCode.

:::note
O OpenAiCode Zen está atualmente em beta.
:::

O Zen funciona como qualquer outro provedor no OpenAiCode. Você faz login no OpenAiCode Zen e obtém sua chave de API. É **completamente opcional** e você não precisa usá-lo para utilizar o OpenAiCode.
...
:::tip
Testamos um grupo selecionado de modelos e provedores que funcionam bem com o OpenAiCode.
:::
...
O OpenAiCode Zen é um gateway de IA que lhe dá acesso a esses modelos.

---

## Como funciona

O OpenAiCode Zen funciona como qualquer outro provedor no OpenAiCode.

1. Você faz login no **<a href={console}>OpenAiCode Zen</a>**, adiciona seus dados de cobrança e copia sua chave de API.
2. Você executa o comando `/connect` no TUI, seleciona OpenAiCode Zen e cola sua chave de API.
...
O [id do modelo](/docs/config/#models) na sua configuração do OpenAiCode usa o formato `opencode/<model-id>`. Por exemplo, para GPT 5.2 Codex, você usaria `opencode/gpt-5.2-codex` na sua configuração.
...
- MiniMax M2.5 Free está disponível no OpenAiCode por tempo limitado. A equipe está usando esse tempo para coletar feedback e melhorar o modelo.
- Big Pickle é um modelo oculto que está gratuito no OpenAiCode por tempo limitado. A equipe está usando esse tempo para coletar feedback e melhorar o modelo.
...
4. Não ter **vinculação** permitindo que você o use com qualquer outro agente de codificação. E sempre permitir que você use qualquer outro provedor com o OpenAiCode também.
