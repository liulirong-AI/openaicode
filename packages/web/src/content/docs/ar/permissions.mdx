...
يستخدم OpenAiCode إعداد `permission` لتحديد ما إذا كان إجراءٌ ما سيُنفّذ تلقائيًا، أو سيطلب موافقتك، أو سيتم حظره.
...
استخدم `external_directory` للسماح باستدعاءات الأدوات التي تلمس مسارات خارج دليل العمل الذي بدأ منه OpenAiCode. ينطبق ذلك على أي أداة تأخذ مسارًا كمدخل (مثل `read` و`edit` و`list` و`glob` و`grep` والعديد من أوامر `bash`).
...
تُعرَّف أذونات OpenAiCode بأسماء الأدوات، بالإضافة إلى بعض حواجز الأمان:
...
إذا لم تحدد شيئًا، يبدأ OpenAiCode بقيم افتراضية متساهلة:
...
عندما يطلب OpenAiCode الموافقة، تعرض الواجهة ثلاث نتائج ممكنة:

- `once` — الموافقة على هذا الطلب فقط
- `always` — الموافقة على الطلبات المستقبلية المطابقة للأنماط المقترحة (لباقي جلسة OpenAiCode الحالية)
- `reject` — رفض الطلب
