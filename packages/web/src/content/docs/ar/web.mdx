---
title: الويب
description: استخدام OpenAiCode في متصفحك.
---

يمكن تشغيل OpenAiCode كتطبيق ويب داخل متصفحك، ليمنحك تجربة البرمجة القوية بالذكاء الاصطناعي نفسها دون الحاجة إلى terminal.

![OpenAiCode Web - جلسة جديدة](../../../assets/web/web-homepage-new-session.png)
...
يؤدي ذلك إلى تشغيل خادم محلي على `127.0.0.1` بمنفذ عشوائي متاح، ويفتح OpenAiCode تلقائيا في المتصفح الافتراضي لديك.
...
افتراضيا، يختار OpenAiCode منفذا متاحا. يمكنك تحديد منفذ:
...
افتراضيا، يرتبط الخادم بـ `127.0.0.1` (للاستخدام المحلي فقط). لجعل OpenAiCode متاحا على شبكتك:

```bash
opencode web --hostname 0.0.0.0
```

عند استخدام `0.0.0.0` سيعرض OpenAiCode كلا من عناوين الوصول المحلي وعناوين الشبكة:
...
بعد التشغيل، تتيح لك واجهة الويب الوصول إلى جلسات OpenAiCode الخاصة بك.

### الجلسات

اعرض جلساتك وأدرها من الصفحة الرئيسية. يمكنك رؤية الجلسات النشطة وبدء جلسات جديدة.

![OpenAiCode Web - جلسة نشطة](../../../assets/web/web-homepage-active-session.png)

### حالة الخادم

انقر على "See Servers" لعرض الخوادم المتصلة وحالتها.

![OpenAiCode Web - See Servers (عرض الخوادم)](../../../assets/web/web-homepage-see-servers.png)
