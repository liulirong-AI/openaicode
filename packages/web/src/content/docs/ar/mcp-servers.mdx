...
يمكنك إضافة أدوات خارجية إلى OpenAiCode باستخدام _Model Context Protocol_، أو MCP. يدعم OpenAiCode خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات OpenAiCode](https://opencode.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يتولى OpenAiCode تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم OpenAiCode بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك OpenAiCode المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `opencode mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن OpenAiCode الرموز بشكل آمن في `~/.local/share/opencode/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل OpenAiCode إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات OpenAiCode مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط OpenAiCode بحساب Sentry الخاص بك.
