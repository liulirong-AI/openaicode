---
title: الإعدادات
description: استخدام ملف إعدادات OpenAiCode بصيغة JSON.
---

يمكنك ضبط OpenAiCode باستخدام ملف إعدادات بصيغة JSON.

---

## التنسيق

يدعم OpenAiCode تنسيقي **JSON** و**JSONC** (JSON مع تعليقات).
...
ضع إعدادات OpenAiCode العامة في `~/.config/opencode/opencode.json`. استخدم الإعدادات العامة للتفضيلات على مستوى المستخدم مثل السمات، والمزوّدين، أو اختصارات المفاتيح.
...
عند تشغيل OpenAiCode، يبحث عن ملف إعدادات في الدليل الحالي أو يصعد حتى أقرب دليل Git.
...
- `mdns` - تفعيل اكتشاف الخدمة عبر mDNS. يتيح ذلك للأجهزة الأخرى على الشبكة اكتشاف خادم OpenAiCode.
...
يمكنك ضبط المزوّدين والنماذج التي تريد استخدامها في إعدادات OpenAiCode عبر الخيارات `provider` و`model` و`small_model`.
...
يضبط الخيار `small_model` نموذجًا منفصلًا للمهام الخفيفة مثل توليد العناوين. افتراضيًا يحاول OpenAiCode استخدام نموذج أقل تكلفة إذا كان متاحًا لدى مزوّدك، وإلا فسيعود إلى النموذج الرئيسي.
...
يمكنك ضبط السمة التي تريد استخدامها في إعدادات OpenAiCode عبر الخيار `theme`.
...
يجب أن يكون الوكيل الافتراضي وكيلًا أساسيًا (وليس وكيلًا فرعيًا). يمكن أن يكون وكيلًا مدمجًا مثل `"build"` أو `"plan"`، أو [وكيلًا مخصصًا](/docs/agents) قمت بتعريفه. إذا لم يكن الوكيل المحدد موجودًا أو كان وكيلًا فرعيًا، فسيعود OpenAiCode إلى `"build"` مع تحذير.
...
سيقوم OpenAiCode بتنزيل أي تحديثات جديدة تلقائيًا عند بدء التشغيل. يمكنك تعطيل ذلك عبر الخيار `autoupdate`.
...
[الإضافات](/docs/plugins) توسّع OpenAiCode بأدوات وخُطافات وتكاملات مخصصة.
...
هذا مفيد عندما تريد تقييد OpenAiCode لاستخدام مزوّدين محددين بدلًا من تعطيلهم واحدًا تلو الآخر.
