---
title: المزوّدون
description: استخدام أي موفّر LLM في OpenAiCode.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم OpenAiCode ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات OpenAiCode.
...
يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات OpenAiCode.
...
OpenAiCode Zen هي قائمة نماذج يوفّرها فريق OpenAiCode وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع OpenAiCode. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع OpenAiCode Zen.
:::
...
يعمل مثل أي مزوّد آخر في OpenAiCode واستخدامه اختياري بالكامل.

---

## OpenAiCode Go

OpenAiCode Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق OpenAiCode، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع OpenAiCode.

1. شغّل الأمر `/connect` في TUI، واختر `OpenAiCode Go`، ثم انتقل إلى [opencode.ai/auth](https://opencode.ai/zen).
...
لاستخدام Amazon Bedrock مع OpenAiCode:
...
:::info
استخدام اشتراك Claude Pro/Max داخل OpenAiCode غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
:::note[ملاحظة امتثال]
يستخدم OpenAiCode نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر OpenAiCode
...
لاستخدام Google Vertex AI مع OpenAiCode:
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) لتسجيل كل محادثة OpenAiCode تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة OpenAiCode مدرجة كجلسة مستقلة.
...
:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع OpenAiCode. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/opencode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع OpenAiCode:
...
3. انسخ مفتاح API لاستخدامه في OpenAiCode.
...
6. **مهم**: قبل استخدام نماذج السحابة في OpenAiCode، يجب سحب معلومات النموذج محليا:
...
OpenAiCode Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق OpenAiCode. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>OpenAiCode Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **OpenAiCode Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ OpenAiCode.
...
تسمح حقول `limit` لـ OpenAiCode بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
