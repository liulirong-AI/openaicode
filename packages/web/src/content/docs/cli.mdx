---
title: CLI
description: OpenAiCode CLI options and commands.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

The OpenAiCode CLI by default starts the [TUI](/docs/tui) when run without any arguments.

```bash
opencode
```

But it also accepts commands as documented on this page. This allows you to interact with OpenAiCode programmatically.
...
Start the OpenAiCode terminal user interface.
...
The OpenAiCode CLI also has the following commands.

---

### agent

Manage agents for OpenAiCode.
...
Attach a terminal to an already running OpenAiCode backend server started via `serve` or `web` commands.

```bash
opencode attach [url]
```

This allows using the TUI with a remote OpenAiCode backend. For example:
...
OpenAiCode is powered by the provider list at [Models.dev](https://models.dev), so you can use `opencode auth login` to configure API keys for any provider you'd like to use. This is stored in `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

When OpenAiCode starts up it loads the providers from the credentials file. And if there are any keys defined in your environments or a `.env` file in your project.
...
Use the `--refresh` flag to update the cached model list. This is useful when new models have been added to a provider and you want to see them in OpenAiCode.
...
Start a headless OpenAiCode server for API access. Check out the [server docs](/docs/server) for the full HTTP interface.
...
Show token usage and cost statistics for your OpenAiCode sessions.
...
Import session data from a JSON file or OpenAiCode share URL.

```bash
opencode import <file>
```

You can import from a local file or an OpenAiCode share URL.
...
Start a headless OpenAiCode server with a web interface.

```bash
opencode web
```

This starts an HTTP server and opens a web browser to access OpenAiCode through a web interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `opencode`).
...
Uninstall OpenAiCode and remove all related files.
...
OpenAiCode can be configured using environment variables.
