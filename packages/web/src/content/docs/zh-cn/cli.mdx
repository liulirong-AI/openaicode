---
title: CLI
description: OpenAiCode CLI 选项和命令。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenAiCode CLI 在不带任何参数运行时，默认启动 [TUI](/docs/tui)。

```bash
opencode
```

但它也接受本页面中记录的命令，使您可以通过编程方式与 OpenAiCode 进行交互。
...
将终端连接到已通过 `serve` 或 `web` 命令启动的 OpenAiCode 后端服务器。

```bash
opencode attach [url]
```

这允许将 TUI 与远程 OpenAiCode 后端配合使用。例如：
...
OpenAiCode 基于 [Models.dev](https://models.dev) 的提供商列表运行，因此您可以使用 `opencode auth login` 为任何想要使用的提供商配置 API 密钥。密钥存储在 `~/.local/share/opencode/auth.json` 中。

```bash
opencode auth login
```

OpenAiCode 启动时会从凭据文件加载提供商信息，同时也会加载环境变量或项目中 `.env` 文件中定义的密钥。
...
使用 `--refresh` 标志可以更新缓存的模型列表。当提供商新增了模型并且您希望在 OpenAiCode 中看到它们时，此功能非常有用。
...
启动无界面的 OpenAiCode 服务器以提供 API 访问。查看[服务器文档](/docs/server)了解完整的 HTTP 接口。

```bash
opencode serve
```

此命令启动一个 HTTP 服务器，提供对 OpenAiCode 功能的 API 访问，无需 TUI 界面。设置 `OPENCODE_SERVER_PASSWORD` 可启用 HTTP 基本认证（用户名默认为 `opencode`）。
...
显示 OpenAiCode 会话的 Token 用量和费用统计信息。
...
从 JSON 文件或 OpenAiCode 分享链接导入会话数据。

```bash
opencode import <file>
```

您可以从本地文件或 OpenAiCode 分享链接导入。
...
启动带有 Web 界面的无界面 OpenAiCode 服务器。

```bash
opencode web
```

此命令启动一个 HTTP 服务器并打开浏览器，通过 Web 界面访问 OpenAiCode。设置 `OPENCODE_SERVER_PASSWORD` 可启用 HTTP 基本认证（用户名默认为 `opencode`）。
