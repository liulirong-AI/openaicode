...
OpenAiCode 内置了两个主代理：**Build** 和 **Plan**。我们将在下面介绍它们。
...
OpenAiCode 内置了两个子代理：**General** 和 **Explore**。我们将在下面介绍它们。

---

## 内置代理

OpenAiCode 内置了两个主代理和两个子代理。
...
如果未指定温度，OpenAiCode 将使用模型特定的默认值；大多数模型通常为 0，Qwen 模型为 0.55。
...
此路径相对于配置文件所在位置。因此它同时适用于全局 OpenAiCode 配置和项目级配置。
...
OpenAiCode 配置中的模型 ID 使用 `provider/model-id` 格式。例如，如果您使用 [OpenAiCode Zen](/docs/zen)，则可以使用 `opencode/gpt-5.1-codex` 来表示 GPT 5.1 Codex。
