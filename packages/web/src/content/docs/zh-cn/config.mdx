---
title: 配置
description: 使用 OpenAiCode JSON 配置。
---

您可以使用 JSON 配置文件来配置 OpenAiCode。

---

## 格式

OpenAiCode 支持 **JSON** 和 **JSONC**（带注释的 JSON）格式。
...
将全局 OpenAiCode 配置放在 `~/.config/opencode/opencode.json` 中。使用全局配置来设置用户级别的偏好，例如主题、提供商或快捷键。
...
当 OpenAiCode 启动时，它会在当前目录中查找配置文件，或向上遍历到最近的 Git 目录。
...
- `mdns` - 启用 mDNS 服务发现。这允许网络上的其他设备发现您的 OpenAiCode 服务器。
...
您可以通过 `provider`、`model` 和 `small_model` 选项在 OpenAiCode 配置中设置要使用的提供商和模型。
...
`small_model` 选项为标题生成等轻量级任务配置单独的模型。默认情况下，如果您的提供商有更便宜的模型可用，OpenAiCode 会尝试使用该模型，否则会回退到您的主模型。
...
您可以通过 OpenAiCode 配置中的 `theme` 选项设置要使用的主题。
...
默认代理必须是主代理（不能是子代理）。可以是内置代理（如 `"build"` 或 `"plan"`），也可以是您定义的[自定义代理](/docs/agents)。如果指定的代理不存在或是子代理，OpenAiCode 将回退到 `"build"` 并发出警告。
...
OpenAiCode 启动时会自动下载新版本。您可以使用 `autoupdate` 选项禁用此功能。
...
默认情况下，OpenAiCode **允许所有操作**，无需明确批准。您可以使用 `permission` 选项更改此行为。
...
[插件](/docs/plugins)通过自定义工具、钩子和集成来扩展 OpenAiCode。
...
当您希望限制 OpenAiCode 仅使用特定提供商，而不是逐一禁用其他提供商时，此选项非常有用。
