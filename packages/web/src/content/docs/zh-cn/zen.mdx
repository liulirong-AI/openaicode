---
title: Zen
description: 由 OpenAiCode 提供的精选模型列表。
---
...
OpenAiCode Zen 是由 OpenAiCode 团队提供的一组经过测试和验证的模型列表。

:::note
OpenAiCode Zen 目前处于测试阶段。
:::

Zen 的工作方式与 OpenAiCode 中的任何其他提供商相同。你只需登录 OpenAiCode Zen 并获取你的 API 密钥。它是**完全可选的**，你无需使用它也能正常使用 OpenAiCode。
...
:::tip
我们测试了一组与 OpenAiCode 配合良好的精选模型和提供商。
:::
...
OpenAiCode Zen 是一个 AI 网关，让你可以访问这些模型。

---

## 工作原理

OpenAiCode Zen 的工作方式与 OpenAiCode 中的任何其他提供商相同。

1. 登录 **<a href={console}>OpenAiCode Zen</a>**，添加你的账单信息，然后复制你的 API 密钥。
2. 在 TUI 中运行 `/connect` 命令，选择 OpenAiCode Zen，然后粘贴你的 API 密钥。
...
在 OpenAiCode 配置中，[模型 ID](/docs/config/#models) 使用 `opencode/<model-id>` 格式。例如，对于 GPT 5.2 Codex，你需要在配置中使用 `opencode/gpt-5.2-codex`。
...
- MiniMax M2.5 Free 在 OpenAiCode 上限时免费提供。团队正在利用这段时间收集反馈并改进模型。
- Big Pickle 是一个隐身模型，在 OpenAiCode 上限时免费提供。团队正在利用这段时间收集反馈并改进模型。
