...
要调试 OpenAiCode 的问题，请先检查其存储在磁盘上的日志和本地数据。
...
OpenAiCode Desktop 会在后台运行一个本地 OpenAiCode 服务器（即 `opencode-cli` 附属进程）。大多数问题是由插件异常、缓存损坏或错误的服务器设置引起的。

### 快速检查

- 完全退出并重新启动应用。
- 如果应用显示错误页面，请点击**重新启动**并复制错误详情。
- 仅限 macOS：`OpenAiCode` 菜单 -> **Reload Webview**（当 UI 空白或冻结时有效）。
...
OpenAiCode 还可以从磁盘加载本地插件。临时将这些插件移走（或重命名文件夹），然后重新启动桌面应用：
...
如果禁用插件没有帮助（或插件安装卡住了），请清除缓存以便 OpenAiCode 重新构建。
...
3. 重新启动 OpenAiCode Desktop。

---

### 修复服务器连接问题

OpenAiCode Desktop 可以启动自己的本地服务器（默认行为），也可以连接到你配置的服务器 URL。
...
在 Windows 上，OpenAiCode Desktop 需要 Microsoft Edge **WebView2 Runtime**。如果应用打开后是空白窗口或无法启动，请安装或更新 WebView2 后重试。

---

### Windows: 常见性能问题

如果你在 Windows 上遇到性能缓慢、文件访问问题或终端问题，请尝试使用 [WSL (Windows Subsystem for Linux)](/docs/windows-wsl)。WSL 提供了一个 Linux 环境，能更好地与 OpenAiCode 的功能兼容。

---

### 通知不显示

OpenAiCode Desktop 仅在以下情况下显示系统通知：

- 在操作系统设置中已为 OpenAiCode 启用通知，且
- 应用窗口未处于焦点状态。
...
2. 找到并删除以下文件（它们位于 OpenAiCode Desktop 应用数据目录中）：
...
如果你遇到 API 调用错误，可能是由于提供商包过期导致的。OpenAiCode 会根据需要动态安装提供商包（OpenAI、Anthropic、Google 等）并将它们缓存到本地。
...
2. 重新启动 OpenAiCode 以重新安装最新的提供商包

这将强制 OpenAiCode 下载最新版本的提供商包，通常可以解决模型参数和 API 变更带来的兼容性问题。
...
OpenAiCode 会检测你是否正在使用 Wayland 并优先使用 `wl-clipboard`，否则将按以下顺序尝试查找剪贴板工具：`xclip` 和 `xsel`。
