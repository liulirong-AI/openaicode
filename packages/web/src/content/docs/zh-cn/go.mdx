...
OpenAiCode Go 是一项低成本的订阅服务（**$10/月**），为您提供对流行开源编程模型的可靠访问。

:::note
OpenAiCode Go 目前处于测试阶段。
:::

Go 的工作方式与 OpenAiCode 中的其他提供商一样。您订阅 OpenAiCode Go 并获取 API 密钥。这是**完全可选的**，您不需要它也能使用 OpenAiCode。
...
:::tip
我们测试了一组精选的模型和提供商，它们与 OpenAiCode 配合良好。
:::
...
OpenAiCode Go 让您可以以 **$10/月** 的价格访问这些模型。

---

## 工作原理

OpenAiCode Go 的工作方式与 OpenAiCode 中的其他提供商一样。

1. 登录 **<a href={console}>OpenAiCode Zen</a>**，订阅 Go，并复制您的 API 密钥。
2. 在 TUI 中运行 `/connect` 命令，选择 `OpenAiCode Go`，然后粘贴您的 API 密钥。
3. 在 TUI 中运行 `/models` 以查看通过 Go 可用的模型列表。

:::note
每个工作区只有一名成员可以订阅 OpenAiCode Go。
:::
...
OpenAiCode Go 是一个 **$10/月** 的订阅计划。以下是**每 1M token** 的价格。
...
OpenAiCode 配置中的 [model id](/docs/config/#models) 使用 `opencode-go/<model-id>` 格式。例如，对于 Kimi K2.5，您将在配置中使用 `opencode-go/kimi-k2.5`。
