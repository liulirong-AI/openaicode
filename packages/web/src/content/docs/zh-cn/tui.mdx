---
title: TUI
description: 使用 OpenAiCode 终端用户界面。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenAiCode 提供了一个交互式终端界面（TUI），用于配合 LLM 处理您的项目。

运行 OpenAiCode 即可启动当前目录的 TUI。
...
使用 OpenAiCode TUI 时，您可以输入 `/` 后跟命令名称来快速执行操作。例如：
...
将提供商添加到 OpenAiCode。允许您从可用的提供商中选择并添加其 API 密钥。
...
您可以通过 OpenAiCode 配置文件自定义 TUI 行为。
