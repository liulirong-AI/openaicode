...
OpenAiCode 使用 [AI SDK](https://ai-sdk.dev/) 和 [Models.dev](https://models.dev)，支持 **75+ LLM 提供商**，同时也支持运行本地模型。

要添加提供商，你需要：

1. 使用 `/connect` 命令添加提供商的 API 密钥。
2. 在 OpenAiCode 配置中设置该提供商。
...
你可以通过 OpenAiCode 配置中的 `provider` 部分来自定义提供商。
...
OpenAiCode Zen 是由 OpenAiCode 团队提供的模型列表，这些模型已经过测试和验证，能够与 OpenAiCode 良好配合使用。[了解更多](/docs/zen)。

:::tip
如果你是新用户，我们建议从 OpenAiCode Zen 开始。
:::
...
它的使用方式与 OpenAiCode 中的其他提供商完全相同，且完全可选。
...
:::info
在 OpenAiCode 中使用 Claude Pro/Max 订阅不是 [Anthropic](https://anthropic.com) 官方支持的用法。
:::
...
2. 前往 [Azure AI Foundry](https://ai.azure.com/) 并部署一个模型。
   ...
   :::note
   部署名称必须与模型名称一致，OpenAiCode 才能正常工作。
   :::
...
:::note
你也可以通过指定 `GITLAB_TOKEN` 环境变量来避免将令牌存储在 OpenAiCode 的认证存储中。
:::

##### 自托管 GitLab

:::note[合规说明]
OpenAiCode 会使用一个小模型来执行部分 AI 任务，例如生成会话标题。默认情况下使用由 Zen 托管的 gpt-5-nano。如果你需要让 OpenAiCode 仅使用你自己的 GitLab 托管实例，请在 `opencode.json` 文件中添加以下内容。同时建议禁用会话共享。
...
要在 OpenAiCode 中使用你的 GitHub Copilot 订阅：
...
如果 Helicone 的某些功能或模型未通过 OpenAiCode 自动配置，你随时可以手动配置。
...
Helicone 的 [Sessions](https://docs.helicone.ai/features/sessions) 功能允许你将相关的 LLM 请求归为一组。使用 [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) 插件可以自动将每个 OpenAiCode 对话记录为 Helicone 中的一个会话。
...
该插件会在你的请求中注入 `Helicone-Session-Id` 和 `Helicone-Session-Name` 请求头。在 Helicone 的 Sessions 页面中，你可以看到每个 OpenAiCode 对话都作为独立的会话列出。
...
你可以通过 [llama.cpp](https://github.com/ggml-org/llama.cpp) 的 llama-server 工具配置 OpenAiCode 使用本地模型。
...
你可以通过 LM Studio 配置 OpenAiCode 使用本地模型。
...
你可以通过 Ollama 配置 OpenAiCode 使用本地模型。

:::tip
Ollama 可以自动为 OpenAiCode 进行配置。详见 [Ollama 集成文档](https://docs.ollama.com/integrations/opencode)。
:::
...
6. **重要**：在 OpenAiCode 中使用云端模型之前，必须先将模型信息拉取到本地：
...
OpenAiCode Zen 是由 OpenAiCode 团队提供的经过测试和验证的模型列表。[了解更多](/docs/zen)。

1. 登录 **<a href={console}>OpenAiCode Zen</a>** 并点击 **Create API Key**。

2. 执行 `/connect` 命令并搜索 **OpenAiCode Zen**。

   ```txt
   /connect
   ```

3. 输入你的 OpenAiCode API 密钥。
...
5. 你还可以通过 OpenAiCode 配置自定义模型。以下是指定提供商的示例：
...
你也可以通过 OpenAiCode 配置自定义模型。以下是指定提供商路由顺序的示例。
...
:::tip
你可以在 OpenAiCode 中使用任何 OpenAI 兼容的提供商。大多数现代 AI 提供商都提供 OpenAI 兼容的 API。
:::
...
`limit` 字段让 OpenAiCode 了解你还剩余多少上下文空间。标准提供商会自动从 models.dev 拉取这些信息。
...
2. 对于自定义提供商，请检查 OpenAiCode 配置并确认：
   - `/connect` 命令中使用的提供商 ID 与 OpenAiCode 配置中的 ID 一致。
