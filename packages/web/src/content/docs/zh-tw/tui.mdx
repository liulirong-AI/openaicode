---
title: TUI
description: 使用 OpenAiCode 終端機使用者介面。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenAiCode 提供了一個互動式終端機介面（TUI），用於配合 LLM 處理您的專案。

執行 OpenAiCode 即可啟動當前目錄的 TUI。
...
使用 OpenAiCode TUI 時，您可以輸入 `/` 後跟指令名稱來快速執行操作。例如：
...
將供應商新增到 OpenAiCode。允許您從可用的供應商中選擇並新增其 API 金鑰。
