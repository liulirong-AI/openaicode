---
title: Web
description: 在瀏覽器中使用 OpenAiCode。
---

OpenAiCode 可以作為 Web 應用程式在瀏覽器中執行，無需終端機即可獲得同樣強大的 AI 編碼體驗。
...
這會在 `127.0.0.1` 上啟動一個本地伺服器，使用隨機可用連接埠，並自動在預設瀏覽器中開啟 OpenAiCode。
...
預設情況下，OpenAiCode 會選擇一個可用連接埠。你也可以指定連接埠：
...
預設情況下，伺服器繫結到 `127.0.0.1`（僅限本地存取）。要使 OpenAiCode 在網路中可存取：

```bash
opencode web --hostname 0.0.0.0
```

使用 `0.0.0.0` 時，OpenAiCode 會同時顯示本地位址和網路位址：
...
啟動後，Web 介面提供對 OpenAiCode 工作階段的存取。
