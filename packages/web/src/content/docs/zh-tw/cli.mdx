---
title: CLI
description: OpenAiCode CLI 選項和指令。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenAiCode CLI 在不帶任何參數執行時，預設啟動 [TUI](/docs/tui)。

```bash
opencode
```

但它也接受本頁面中記錄的指令，使您可以透過程式方式與 OpenAiCode 進行互動。
...
將終端機連接到已透過 `serve` 或 `web` 指令啟動的 OpenAiCode 後端伺服器。

```bash
opencode attach [url]
```

這允許將 TUI 與遠端 OpenAiCode 後端搭配使用。例如：
...
OpenAiCode 基於 [Models.dev](https://models.dev) 的供應商列表運作，因此您可以使用 `opencode auth login` 為任何想要使用的供應商設定 API 金鑰。金鑰儲存在 `~/.local/share/opencode/auth.json` 中。

```bash
opencode auth login
```

OpenAiCode 啟動時會從憑證檔案載入供應商資訊，同時也會載入環境變數或專案中 `.env` 檔案中定義的金鑰。
...
使用 `--refresh` 旗標可以更新快取的模型列表。當供應商新增了模型並且您希望在 OpenAiCode 中看到它們時，此功能非常有用。
...
啟動無介面的 OpenAiCode 伺服器以提供 API 存取。查看[伺服器文件](/docs/server)了解完整的 HTTP 介面。

```bash
opencode serve
```

此指令啟動一個 HTTP 伺服器，提供對 OpenAiCode 功能的 API 存取，無需 TUI 介面。設定 `OPENCODE_SERVER_PASSWORD` 可啟用 HTTP 基本認證（使用者名稱預設為 `opencode`）。
...
顯示 OpenAiCode 工作階段的 Token 用量和費用統計資訊。
...
從 JSON 檔案或 OpenAiCode 分享連結匯入工作階段資料。

```bash
opencode import <file>
```

您可以從本地檔案或 OpenAiCode 分享連結匯入。
...
啟動帶有 Web 介面的無介面 OpenAiCode 伺服器。

```bash
opencode web
```

此指令啟動一個 HTTP 伺服器並開啟瀏覽器，透過 Web 介面存取 OpenAiCode。設定 `OPENCODE_SERVER_PASSWORD` 可啟用 HTTP 基本認證（使用者名稱預設為 `opencode`）。
