...
要偵錯 OpenAiCode 的問題，請先檢查其儲存在磁碟上的日誌和本地資料。
...
OpenAiCode 將工作階段資料和其他應用程式資料儲存在磁碟上：
...
OpenAiCode Desktop 會在背景執行一個本地 OpenAiCode 伺服器（即 `opencode-cli` 附屬程序）。大多數問題是由外掛異常、快取損壞或錯誤的伺服器設定引起的。

### 快速檢查

- 完全退出並重新啟動應用程式。
- 如果應用程式顯示錯誤頁面，請點擊**重新啟動**並複製錯誤詳情。
- 僅限 macOS：`OpenAiCode` 選單 -> **Reload Webview**（當 UI 空白或凍結時有效）。
...
OpenAiCode 還可以從磁碟載入本地外掛。暫時將這些外掛移走（或重新命名資料夾），然後重新啟動桌面應用程式：
...
如果停用外掛沒有幫助（或外掛安裝卡住了），請清除快取以便 OpenAiCode 重新建置。

1. 完全退出 OpenAiCode Desktop。
2. 刪除快取目錄：
...
3. 重新啟動 OpenAiCode Desktop。

---

### 修復伺服器連線問題

OpenAiCode Desktop 可以啟動自己的本地伺服器（預設行為），也可以連線到你設定的伺服器 URL。
...
在 Windows 上，OpenAiCode Desktop 需要 Microsoft Edge **WebView2 Runtime**。如果應用程式開啟後是空白視窗或無法啟動，請安裝或更新 WebView2 後重試。

---

### Windows: 常見效能問題

如果你在 Windows 上遇到效能緩慢、檔案存取問題或終端機問題，請嘗試使用 [WSL (Windows Subsystem for Linux)](/docs/windows-wsl)。WSL 提供了一個 Linux 環境，能更好地與 OpenAiCode 的功能相容。

---

### 通知不顯示

OpenAiCode Desktop 僅在以下情況下顯示系統通知：

- 在作業系統設定中已為 OpenAiCode 啟用通知，且
- 應用程式視窗未處於焦點狀態。
...
1. 退出 OpenAiCode Desktop。
2. 找到並刪除以下檔案（它們位於 OpenAiCode Desktop 應用程式資料目錄中）：
...
如果你遇到 API 呼叫錯誤，可能是由於供應商套件過期導致的。OpenAiCode 會根據需要動態安裝供應商套件（OpenAI、Anthropic、Google 等）並將它們快取到本地。
...
2. 重新啟動 OpenAiCode 以重新安裝最新的供應商套件

這將強制 OpenAiCode 下載最新版本的供應商套件，通常可以解決模型參數和 API 變更帶來的相容性問題。
...
OpenAiCode 會偵測你是否正在使用 Wayland 並優先使用 `wl-clipboard`，否則將按以下順序嘗試查找剪貼簿工具：`xclip` 和 `xsel`。
