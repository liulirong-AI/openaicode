---
title: 設定
description: 使用 OpenAiCode JSON 設定。
---

您可以使用 JSON 設定檔來設定 OpenAiCode。

---

## 格式

OpenAiCode 支援 **JSON** 和 **JSONC**（帶註解的 JSON）格式。
...
將全域 OpenAiCode 設定放在 `~/.config/opencode/opencode.json` 中。使用全域設定來設定使用者層級的偏好，例如主題、供應商或快捷鍵。
...
當 OpenAiCode 啟動時，它會在當前目錄中尋找設定檔，或向上遍歷到最近的 Git 目錄。
...
- `mdns` - 啟用 mDNS 服務探索。這允許網路上的其他裝置發現您的 OpenAiCode 伺服器。
...
您可以透過 `provider`、`model` 和 `small_model` 選項在 OpenAiCode 設定中設定要使用的供應商和模型。
...
`small_model` 選項為標題生成等輕量級任務設定單獨的模型。預設情況下，如果您的供應商有更便宜的模型可用，OpenAiCode 會嘗試使用該模型，否則會回退到您的主模型。
...
預設代理必須是主代理（不能是子代理）。可以是內建代理（如 `"build"` 或 `"plan"`），也可以是您定義的[自訂代理](/docs/agents)。如果指定的代理不存在或是子代理，OpenAiCode 將回退到 `"build"` 並發出警告。
...
OpenAiCode 啟動時會自動下載新版本。您可以使用 `autoupdate` 選項停用此功能。
...
預設情況下，OpenAiCode **允許所有操作**，無需明確批准。您可以使用 `permission` 選項變更此行為。
...
[外掛程式](/docs/plugins)透過自訂工具、掛鉤和整合來擴展 OpenAiCode。
...
當您希望限制 OpenAiCode 僅使用特定供應商，而不是逐一停用其他供應商時，此選項非常有用。
