...
Možete dodati vanjske alate u OpenAiCode koristeći _Model Context Protocol_, ili MCP. OpenAiCode podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [OpenAiCode Config](https://opencode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
OpenAiCode automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, OpenAiCode će:
...
Ako server zahtijeva autentifikaciju, OpenAiCode će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `opencode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, OpenAiCode sigurno čuva tokene u `~/.local/share/opencode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u OpenAiCode, zajedno s ugrađenim alatima. Možete njima upravljati kroz OpenAiCode konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete OpenAiCode sa vašim Sentry nalogom.
