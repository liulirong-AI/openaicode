---
title: CLI
description: OpenAiCode CLI opcije i naredbe.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenAiCode CLI po defaultu pokreće [TUI](/docs/tui) kada se pokrene bez ikakvih argumenata.

```bash
opencode
```

Ali takođe prihvata naredbe kao što je dokumentovano na ovoj stranici. Ovo vam omogućava programsku interakciju sa OpenAiCode.
...
Pokrenite OpenAiCode terminalski korisnički interfejs.
...
OpenAiCode CLI takođe ima sljedeće naredbe.

---

### agent

Upravljajte OpenAiCode agentima.
...
Priključite terminal na već pokrenut OpenAiCode backend server pokrenut putem `serve` ili `web` naredbi.

```bash
opencode attach [url]
```

Ovo omogućava korištenje TUI-ja sa udaljenim OpenAiCode backend-om. Na primjer:
...
OpenAiCode pokreće lista provajdera na [Models.dev](https://models.dev), tako da možete koristiti `opencode auth login` da konfigurirate API ključeve za bilo kojeg provajdera kojeg želite koristiti. Ovo je pohranjeno u `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Kada se OpenAiCode pokrene, učitava dobavljače iz datoteke vjerodajnica. I ako postoje neki ključevi definirani u vašim okruženjima ili `.env` fajl u vašem projektu.
...
Koristite `--refresh` zastavicu da ažurirate keširanu listu modela. Ovo je korisno kada su novi modeli dodani provajderu i želite da ih vidite u OpenAiCode.
...
Pokrenite OpenAiCode u neinteraktivnom modu tako što ćete direktno proslijediti prompt.
...
| `--attach`   |        | Priključite na pokrenuti OpenAiCode server (npr. http://localhost:4096)    |
...
Pokrenite OpenAiCode headless server za API pristup. Pogledajte [server docs](/docs/server) za kompletan HTTP interfejs.

```bash
opencode serve
```

Ovo pokreće HTTP server koji pruža API pristup funkcionalnosti OpenAiCode-a bez TUI interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `opencode`).
...
Upravljajte OpenAiCode sesijama.
...
Navedite sve OpenAiCode sesije.
...
Prikaži statistiku upotrebe tokena i troškova za vaše OpenAiCode sesije.
...
Uvezite podatke sesije iz JSON datoteke ili OpenAiCode dijeljenog URL-a.

```bash
opencode import <file>
```

Možete uvesti iz lokalne datoteke ili OpenAiCode dijeljenog URL-a.
...
Pokrenite OpenAiCode headless server sa web interfejsom.

```bash
opencode web
```

Ovo pokreće HTTP server i otvara web pretraživač za pristup OpenAiCode-u preko web interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `opencode`).
...
Deinstalirajte OpenAiCode i uklonite sve povezane datoteke.
...
Ažurira OpenAiCode na najnoviju verziju ili određenu verziju.
...
OpenAiCode CLI prihvata sljedeće globalne zastavice.
...
OpenAiCode se može konfigurirati pomoću varijabli okruženja.
