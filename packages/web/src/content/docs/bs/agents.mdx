...
Postoje dvije vrste agenata u OpenAiCode: primarni agenti i podagenti.
...
OpenAiCode dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
OpenAiCode dolazi sa dva ugrađena subagenta, **General** i **Explore**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

OpenAiCode dolazi sa dva ugrađena primarna agenta i dva ugrađena subagenta.
...
Ako temperatura nije navedena, OpenAiCode koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu OpenAiCode konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj OpenAiCode konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [OpenAiCode Zen](/docs/zen), koristili biste `opencode/gpt-5.1-codex` za GPT 5.1 Codex.
