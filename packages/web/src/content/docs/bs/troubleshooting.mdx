...
Da biste otklonili probleme s OpenAiCode, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
OpenAiCode pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:
...
OpenAiCode Desktop pokreće lokalni OpenAiCode server (`opencode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `OpenAiCode` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
OpenAiCode također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga OpenAiCode mogao ponovo izgraditi.

1. Potpuno zatvorite OpenAiCode Desktop.
...
3. Ponovo pokrenite OpenAiCode Desktop.

---

### Rješavanje problema sa vezom na serveru

OpenAiCode Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Na Windows-u, OpenAiCode Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa OpenAiCode karakteristikama.

---

### Obavještenja se ne prikazuju

OpenAiCode Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za OpenAiCode u postavkama vašeg OS-a, i
...
1. Zatvorite OpenAiCode Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka OpenAiCode Desktop aplikacije):
...
Ako imate problema s OpenAiCode:
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. OpenAiCode dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
2. Ponovo pokrenite OpenAiCode da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti OpenAiCode da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
OpenAiCode će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
