...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. OpenAiCode dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u OpenAiCode.
...
:::note
Ovaj alat je dostupan samo uz OpenAiCode provajdera ili kada je varijabla `OPENCODE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju OpenAiCode:
