---
title: Konfiguracija
description: Korištenje OpenAiCode JSON konfiguracije.
---

Možete konfigurirati OpenAiCode koristeći JSON konfiguracijski fajl.

---

## Format

OpenAiCode podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
Postavite svoju globalnu OpenAiCode konfiguraciju u `~/.config/opencode/opencode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.
...
Kada se OpenAiCode pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš OpenAiCode server.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj OpenAiCode konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, OpenAiCode pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, OpenAiCode će se vratiti na `"build"` s upozorenjem.
...
OpenAiCode će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, OpenAiCode **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju OpenAiCode sa prilagođenim alatima, kukicama i integracijama.
...
Ovo je korisno kada želite da ograničite OpenAiCode da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
