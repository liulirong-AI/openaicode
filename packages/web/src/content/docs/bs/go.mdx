...
OpenAiCode Go je povoljna pretplata od **$10/mjesečno** koja vam daje pouzdan pristup popularnim otvorenim modelima kodiranja.

:::note
OpenAiCode Go je trenutno u beta fazi.
:::

Go funkcioniše kao bilo koji drugi pružatelj usluga u OpenAiCode-u. Pretplatite se na OpenAiCode Go i
dobijete svoj API ključ. To je **potpuno opcionalno** i ne morate ga koristiti da biste
koristili OpenAiCode.
...
:::tip
Testirali smo odabranu grupu modela i pružatelja usluga koji dobro rade sa OpenAiCode-om.
:::
...
OpenAiCode Go vam daje pristup ovim modelima za **$10/mjesečno**.

---

## Kako to funkcioniše

OpenAiCode Go funkcioniše kao bilo koji drugi pružatelj usluga u OpenAiCode-u.

1. Prijavite se na **<a href={console}>OpenAiCode Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite `/connect` komandu u TUI-ju, odaberite `OpenAiCode Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih putem Go.

:::note
Samo jedan član po radnom prostoru se može pretplatiti na OpenAiCode Go.
:::
...
OpenAiCode Go uključuje sljedeća ograničenja:
...
OpenAiCode Go je pretplatnički plan od **$10/mjesečno**. Ispod su cijene **po 1M tokena**.
...
[Model id](/docs/config/#models) u vašoj OpenAiCode konfiguraciji
...
4. Nemamo **zaključavanja (lock-in)** omogućavajući vam da koristite bilo kojeg drugog pružatelja usluga sa OpenAiCode-om također.
