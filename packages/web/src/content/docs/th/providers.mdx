---
title: ผู้ให้บริการ
description: ใช้ผู้ให้บริการ LLM ใน OpenAiCode
---

import config from "../../../../config.mjs"
export const console = config.console

OpenAiCode ใช้ [AI SDK](https://ai-sdk.dev/) และ [Models.dev](https://models.dev) เพื่อรองรับผู้ให้บริการ **75+ LLM** และรองรับการใช้งานโมเดลท้องถิ่น

หากต้องการเพิ่มผู้ให้บริการ คุณต้อง:

1. เพิ่มคีย์ API สำหรับผู้ให้บริการโดยใช้คำสั่ง `/connect`
2. กำหนดค่าผู้ให้บริการในการกำหนดค่า OpenAiCode ของคุณ
...
คุณสามารถปรับแต่งผู้ให้บริการได้ผ่านทางส่วน `provider` ใน OpenAiCode ของคุณ
กำหนดค่า
...
OpenAiCode Zen คือรายชื่อโมเดลที่จัดทำโดยทีมงาน OpenAiCode ที่ได้รับ
ทดสอบและตรวจสอบแล้วว่าทำงานได้ดีกับ OpenAiCode [เรียนรู้เพิ่มเติม](/docs/zen)

:::tip
หากคุณยังใหม่ เราขอแนะนำให้เริ่มต้นด้วย OpenAiCode Zen
:::
...
มันทำงานเหมือนกับผู้ให้บริการรายอื่นใน OpenAiCode และเป็นทางเลือกในการใช้งานโดยสมบูรณ์

---

## OpenAiCode Go

OpenAiCode Go คือแผนการสมัครสมาชิกราคาประหยัดที่ให้การเข้าถึงโมเดลการเขียนโค้ดแบบเปิดยอดนิยมที่เชื่อถือได้ ซึ่งจัดเตรียมโดยทีมงาน OpenAiCode ที่ได้รับการทดสอบและตรวจสอบแล้วว่าทำงานได้ดีกับ OpenAiCode

1. เรียกใช้คำสั่ง `/connect` ใน TUI เลือก `OpenAiCode Go` และไปที่ [opencode.ai/auth](https://opencode.ai/zen)
...
วิธีใช้ Amazon Bedrock กับ OpenAiCode:
...
:::info
การใช้การสมัครสมาชิก Claude Pro/Max ใน OpenAiCode ไม่ได้รับการสนับสนุนอย่างเป็นทางการจาก [Anthropic](https://anthropic.com)
:::
...
OpenAiCode ใช้โมเดลขนาดเล็กสำหรับงาน AI บางอย่าง เช่น การสร้างชื่อเซสชัน
มีการกำหนดค่าให้ใช้ gpt-5-nano ตามค่าเริ่มต้น ซึ่งโฮสต์โดย Zen เพื่อล็อค OpenAiCode
...
วิธีใช้ Google Vertex AI กับ OpenAiCode:
...
คุณลักษณะ [เซสชัน](https://docs.helicone.ai/features/sessions) ของ Helicone ช่วยให้คุณสามารถจัดกลุ่มคำขอ LLM ที่เกี่ยวข้องเข้าด้วยกันได้ ใช้ปลั๊กอิน [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) เพื่อบันทึกการสนทนา OpenAiCode แต่ละรายการเป็นเซสชันใน Helicone โดยอัตโนมัติ
...
ปลั๊กอินจะแทรกส่วนหัว `Helicone-Session-Id` และ `Helicone-Session-Name` ลงในคำขอของคุณ ในหน้าเซสชันของ Helicone คุณจะเห็นการสนทนา OpenAiCode แต่ละรายการแยกเป็นเซสชันแยกกัน
...
:::tip
Ollama สามารถกำหนดค่าตัวเองสำหรับ OpenAiCode ได้โดยอัตโนมัติ ดู [เอกสารการบูรณาการ Ollama](https://docs.ollama.com/integrations/opencode) เพื่อดูรายละเอียด
:::
...
วิธีใช้ Ollama Cloud กับ OpenAiCode:
...
3. คัดลอกคีย์ API เพื่อใช้ใน OpenAiCode
...
6. **สำคัญ**: ก่อนที่จะใช้โมเดลคลาวด์ใน OpenAiCode คุณต้องดึงข้อมูลโมเดลในเครื่อง:
...
OpenAiCode Zen คือรายการโมเดลที่ได้รับการทดสอบและตรวจสอบแล้วโดยทีมงาน OpenAiCode [เรียนรู้เพิ่มเติม](/docs/zen)

1. ลงชื่อเข้าใช้ **<a href={console}>OpenAiCode Zen</a>** และคลิก **สร้าง API คีย์**

2. เรียกใช้คำสั่ง `/connect` และค้นหา **OpenAiCode Zen**

   ```txt
   /connect
   ```

3. ป้อนคีย์ OpenAiCode API ของคุณ
...
ฟิลด์ `limit` ช่วยให้ OpenAiCode เข้าใจว่าคุณมีบริบทเหลืออยู่เท่าใด ผู้ให้บริการมาตรฐานจะดึงข้อมูลเหล่านี้จาก models.dev โดยอัตโนมัติ
...
ช่อง `limit` ช่วยให้ OpenAiCode เข้าใจบริบทที่คุณเหลืออยู่ ผู้ให้บริการมาตรฐานจะดึงข้อมูลเหล่านี้จาก models.dev โดยอัตโนมัติ
