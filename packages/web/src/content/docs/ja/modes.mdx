...
:::caution
モードは、OpenAiCode 設定の `agent` オプションを通じて設定されるようになりました。
...
これらを使用するか、OpenAiCode 設定を通じて独自の設定を行います。
...
OpenAiCode には 2 つの組み込みモードが付属しています。
...
温度が指定されていない場合、OpenAiCode はモデル固有のデフォルトを使用します (通常、ほとんどのモデルでは 0、Qwen モデルでは 0.55)。
