---
title: IDE
description: VS Code、Cursor、およびその他の IDE 用の OpenAiCode 拡張機能
---

OpenAiCode は、VS Code、Cursor、またはターミナルをサポートする任意の IDE と統合します。開始するには、ターミナルで `opencode` を実行するだけです。

---

## 使用法

- **クイック起動**: `Cmd+Esc` (Mac) または `Ctrl+Esc` (Windows/Linux) を使用して、分割ターミナルビューで OpenAiCode を開くか、既存のターミナルセッションが既に実行されている場合はそれにフォーカスします。
- **新しいセッション**: すでに開いている場合でも、`Cmd+Shift+Esc` (Mac) または `Ctrl+Shift+Esc` (Windows/Linux) を使用して、新しい OpenAiCode ターミナルセッションを開始します。 UI の [OpenAiCode] ボタンをクリックすることもできます。
- **コンテキスト認識**: 現在の選択またはタブを OpenAiCode と自動的に共有します。
...
OpenAiCode を VS Code および Cursor、Windsurf、VSCodium などの一般的なフォークにインストールするには:
...
Extension Marketplace で **OpenAiCode** を検索し、**インストール** をクリックします。
