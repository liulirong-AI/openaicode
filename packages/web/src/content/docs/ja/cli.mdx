---
title: CLI
description: OpenAiCode CLI のオプションとコマンド。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenAiCode CLI は、引数なしで実行すると、デフォルトで [TUI](/docs/tui) を開始します。

```bash
opencode
```

ただし、このページに記載されているようにコマンドも受け入れます。これにより、OpenAiCode をプログラム的に操作できるようになります。
...
OpenAiCode ターミナルユーザーインターフェイスを開始します。
...
OpenAiCode CLI には次のコマンドもあります。

---

### agent

OpenAiCode のエージェントを管理します。
...
`serve` または `web` コマンドを使用して起動された、すでに実行中の OpenAiCode バックエンドサーバーにターミナルを接続します。

```bash
opencode attach [url]
```

これにより、リモート OpenAiCode バックエンドで TUI を使用できるようになります。例えば：
...
OpenAiCode は [Models.dev](https://models.dev) のプロバイダーリストを利用しているため、`opencode auth login` を使用して、使用したいプロバイダーの API キーを構成できます。これは `~/.local/share/opencode/auth.json` に保存されます。

```bash
opencode auth login
```

OpenAiCode が起動すると、認証情報ファイルからプロバイダーがロードされます。また、環境またはプロジェクト内の `.env` ファイルで定義されたキーがあるかどうかも確認します。
...
`--refresh` フラグを使用して、キャッシュされたモデルリストを更新します。これは、新しいモデルがプロバイダーに追加され、それを OpenAiCode で確認したい場合に便利です。
...
API アクセスのためにヘッドレス OpenAiCode サーバーを起動します。完全な HTTP インターフェイスについては、[サーバーのドキュメント](/docs/server) を確認してください。
...
OpenAiCode セッションのトークンの使用状況とコストの統計を表示します。
...
JSON ファイルまたは OpenAiCode 共有 URL からセッションデータをインポートします。

```bash
opencode import <file>
```

ローカルファイルまたは OpenAiCode 共有 URL からインポートできます。
...
Web インターフェイスを使用してヘッドレス OpenAiCode サーバーを起動します。

```bash
opencode web
```

これにより、HTTP サーバーが起動し、Web ブラウザが開き、Web インターフェイスを通じて OpenAiCode にアクセスします。 `OPENCODE_SERVER_PASSWORD` を設定して HTTP 基本認証を有効にします (ユーザー名のデフォルトは `opencode`)。
...
OpenAiCode をアンインストールし、関連ファイルをすべて削除します。
