...
エージェントスキルにより、OpenAiCode はリポジトリまたはホームディレクトリから再利用可能な命令を検出できます。
...
プロジェクトのローカルパスの場合、OpenAiCode は現在の作業ディレクトリから git ワークツリーに到達するまで進みます。
...
OpenAiCode では、`skill` ツールの説明に利用可能なスキルがリストされています。
