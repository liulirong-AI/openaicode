...
_Model Context Protocol_ (MCP) を使用して、OpenAiCode に外部ツールを追加できます。 OpenAiCode は、ローカルサーバーとリモートサーバーの両方をサポートします。
...
MCP サーバーは、`mcp` の下の [OpenAiCode 設定](https://opencode.ai/docs/config/) で定義できます。各 MCP を一意の名前で追加します。 LLM にプロンプ​​トを表示するときに、その MCP を名前で参照できます。
...
OpenAiCode は、リモート MCP サーバーの OAuth 認証を自動的に処理します。サーバーが認証を必要とする場合、OpenAiCode は次のことを行います。
...
サーバーが認証を必要とする場合、OpenAiCode を初めて使用しようとすると、認証を求めるプロンプトが表示されます。そうでない場合は、`opencode mcp auth <server-name>` を使用して [flow](#authenticating) を手動でトリガーできます。
...
`mcp auth` コマンドは、認証のためにブラウザを開きます。承認後、OpenAiCode はトークンを `~/.local/share/opencode/mcp-auth.json` に安全に保存します。
...
MCP は、組み込みツールと並んで、OpenAiCode のツールとして利用できます。したがって、他のツールと同様に、OpenAiCode 設定を通じてそれらを管理できます。
...
これにより、ブラウザウィンドウが開き、OAuth フローが完了し、OpenAiCode が Sentry アカウントに接続されます。
