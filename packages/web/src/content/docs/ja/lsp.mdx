---
title: LSPサーバー
description: OpenAiCode は LSP サーバーと統合します。
---

OpenAiCode は言語サーバープロトコル (LSP) と統合して、LLM がコードベースと対話できるようにします。診断を使用して LLM にフィードバックを提供します。

---

## 組み込み

OpenAiCode には、一般的な言語用のいくつかの組み込み LSP サーバーが付属しています。
...
OpenAiCode がファイルを開くと、次のようになります。
...
OpenAiCode 設定の `lsp` セクションを通じて LSP サーバーをカスタマイズできます。
