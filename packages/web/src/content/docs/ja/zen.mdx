---
title: OpenAiCode Zen
description: OpenAiCode によって提供されるモデルの厳選されたリスト。
---
...
OpenAiCode Zen は、OpenAiCode チームによって提供される、テストおよび検証されたモデルのリストです。

:::note
OpenAiCode Zen は現在ベータ版です。
:::
Zen は OpenAiCode の他のプロバイダーと同様に機能します。 OpenAiCode Zen にログインすると、API キーを取得できます。これは **完全にオプション** であり、OpenAiCode を使用するために使用する必要はありません。
...
:::tip
OpenAiCode で適切に動作する、厳選されたモデルとプロバイダーのグループをテストしました。
:::
...
OpenAiCode Zen は、これらのモデルへのアクセスを可能にする AI ゲートウェイです。

---

## 仕組み

OpenAiCode Zen は、OpenAiCode の他のプロバイダーと同様に機能します。

1. **<a href={console}>OpenAiCode Zen</a>** にサインインし、請求情報を追加して、API キーをコピーします。
2. TUI で `/connect` コマンドを実行し、OpenAiCode Zen を選択して API キーを貼り付けます。
...
OpenAiCode 設定の [model id](/docs/config/#models)
...
- MiniMax M2.5 Free は期間限定で OpenAiCode で利用可能です。チームはこの期間を利用してフィードバックを収集し、モデルを改善します。
- Big Pickle は、期間限定で OpenAiCode で無料で利用できるステルスモデルです。チームはこの期間を利用してフィードバックを収集し、モデルを改善します。
...
私たちは次の目的で OpenAiCode Zen を作成しました。
...
4. 他のコーディングエージェントとの併用を許可することで、**ロックイン**がなくなります。また、常に OpenAiCode で他のプロバイダーも使用できるようにします。
