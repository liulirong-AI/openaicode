---
title: サーバー
description: HTTP 経由で OpenAiCode サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`opencode serve` コマンドは、OpenAiCode クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
:::tip
OpenAiCode サーバーを使用して、プログラムで OpenAiCode と対話します。
:::

`opencode serve` を実行してスタンドアロンサーバーを起動できます。すでに OpenAiCode TUI を実行している場合でも、`opencode serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、OpenAiCode [IDE](/docs/ide) プラグインによって使用されます。
