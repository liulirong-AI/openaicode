...
OpenAiCode has a list of keybinds that you can customize through `tui.json`.
...
OpenAiCode uses a `leader` key for most keybinds. This avoids conflicts in your terminal.
...
The OpenAiCode desktop app prompt input supports common Readline/Emacs-style shortcuts for editing text. These are built-in and currently not configurable via `opencode.json`.
