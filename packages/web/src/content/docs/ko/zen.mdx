---
title: Zen
description: OpenAiCode가 제공하는 엄선된 모델 목록.
---
...
OpenAiCode Zen은 OpenAiCode 팀이 제공하는, 테스트 및 검증을 완료한 모델 목록입니다.

:::note
OpenAiCode Zen은 현재 베타(Beta) 단계에 있습니다.
:::

Zen은 OpenAiCode 내의 다른 공급자와 동일한 방식으로 작동합니다. 사용자는 OpenAiCode Zen에 로그인하여 API 키를 발급받을 수 있습니다. 본 서비스는 **전적으로 선택 사항**이며, OpenAiCode를 사용하기 위해 반드시 OpenAiCode Zen을 이용할 필요는 없습니다.
...
:::tip
OpenAiCode와 함께 원활하게 작동하는 일부 모델 및 제공자를 선별하여 테스트를 진행하였습니다.
:::
...
OpenAiCode Zen은 이러한 모델에 대한 접근을 제공하는 AI 게이트웨이입니다.

---

## 어떻게 작동하나요?

OpenAiCode Zen은 OpenAiCode의 다른 제공자와 동일한 방식으로 작동합니다.

1. **<a href={console}>OpenAiCode Zen</a>**에 로그인한 후, 결제 정보를 추가하고 API 키를 복사합니다.
2. TUI에서 `/connect` 명령어를 실행한 뒤, OpenAiCode Zen을 선택하고 API 키를 붙여넣습니다.
...
OpenAiCode 설정 파일에서 사용하는 [모델 ID](/docs/config/#models)는 `opencode/<model-id>` 형식을 따릅니다.
...
- MiniMax M2.5 Free는 한정된 기간 동안 OpenAiCode에서 제공됩니다. 해당 기간 동안 팀은 사용자 피드백을 수집하고 모델을 개선할 예정입니다.
- Big Pickle은 한정된 기간 동안 OpenAiCode에서 무료로 제공되는 스텔스 모델입니다. 해당 기간 동안 팀은 사용자 피드백을 수집하고 모델을 개선할 예정입니다.
...
OpenAiCode Zen은 다음과 같은 목표를 바탕으로 개발되었습니다.
...
4. 특정 서비스에 종속되지 않도록 하여, 다른 코딩 에이전트와도 자유롭게 함께 사용할 수 있도록 하며, OpenAiCode 내에서 다른 제공자 역시 언제든지 사용할 수 있도록 합니다.
