...
OpenAiCode의 agent는 primary agent와 subagent, 두 가지 유형으로 나뉩니다.
...
OpenAiCode에는 기본 제공 primary agent인 **Build**와 **Plan**이 포함되어 있습니다. 아래에서 각각 살펴보겠습니다.
...
OpenAiCode에는 기본 제공 subagent인 **General**과 **Explore**가 포함되어 있습니다. 아래에서 살펴보겠습니다.

---

## 기본 제공

OpenAiCode는 기본적으로 primary agent 2개와 subagent 2개를 제공합니다.
...
temperature를 지정하지 않으면 OpenAiCode는 model별 기본값을 사용합니다. 일반적으로 대부분의 model은 0, Qwen model은 0.55를 사용합니다.
...
이 경로는 config 파일 위치 기준의 상대 경로입니다. 따라서 전역 OpenAiCode config와 프로젝트별 config 모두에서 동일하게 동작합니다.
...
OpenAiCode config의 model ID는 `provider/model-id` 형식을 사용합니다. 예를 들어 [OpenAiCode Zen](/docs/zen)을 사용한다면 GPT 5.1 Codex에 `opencode/gpt-5.1-codex`를 사용합니다.
