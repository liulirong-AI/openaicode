...
For at fejlfinde problemer med OpenAiCode, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
OpenAiCode Desktop kører en lokal OpenAiCode-server (`opencode-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `OpenAiCode`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
OpenAiCode kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så OpenAiCode kan genopbygge den.

1. Afslut OpenAiCode Desktop helt.
2. Slet cache-mappen:
...
3. Start OpenAiCode Desktop på ny.

---

### Løs problemer med serverforbindelse

OpenAiCode Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
På Windows kræver OpenAiCode Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i OpenAiCode.

---

### Meddelelser vises ikke

OpenAiCode Desktop viser kun systemvarsler når:

- varsler er aktiveret for OpenAiCode i dine OS-indstillinger, og
...
1. Afslut OpenAiCode Desktop.
2. Find og slet disse filer (de findes i OpenAiCode Desktop-appens datamappe):
...
Hvis du oplever problemer med OpenAiCode:
