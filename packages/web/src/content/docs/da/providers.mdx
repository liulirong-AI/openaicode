---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i OpenAiCode.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenAiCode bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i OpenAiCode-konfigurationen.
...
Du kan tilpasse udbyderne gennem `provider`-delen i OpenAiCode-konfigurationen.
...
OpenAiCode Zen er en liste over modeller leveret af OpenAiCode-teamet, som er blevet testet og verificeret til at fungere godt med OpenAiCode. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med OpenAiCode Zen.
:::
...
Det fungerer som alle andre udbydere i OpenAiCode og er helt valgfrit at bruge.

---

## OpenAiCode Go

OpenAiCode Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af OpenAiCode-teamet, som er testet og verificeret til at fungere godt med OpenAiCode.

1. Kør kommandoen `/connect` i TUI, vælg `OpenAiCode Go`, og gå til [opencode.ai/auth](https://opencode.ai/zen).
...
Sådan bruger du Amazon Bedrock med OpenAiCode:
...
:::info
At bruge dit Claude Pro/Max-abonnement i OpenAiCode understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
OpenAiCode bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse OpenAiCode
...
Sådan bruger du GitHub Copilot-abonnementet med OpenAiCode:
...
Sådan bruger du Google Vertex AI med OpenAiCode:
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk at logge hver OpenAiCode-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver OpenAiCode-samtale opført som en separat session.
...
:::tip
Ollama kan automatisk konfigurere sig selv for OpenAiCode. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med OpenAiCode:
...
3. Kopier API-nøglen til brug i OpenAiCode.
...
6. **Vigtigt**: Før du bruger skymodeller i OpenAiCode, skal du hente modelinformationen lokalt:
...
OpenAiCode Zen er en liste over testede og verificerede modeller leveret af OpenAiCode-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>OpenAiCode Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **OpenAiCode Zen**.

   ```txt
   /connect
   ```

3. Indtast OpenAiCode API-nøglen.
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med OpenAiCode:
...
`limit`-felterne lader OpenAiCode forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
