...
OpenAiCode Go er et billigt **$10/måned** abonnement, der giver dig pålidelig adgang til populære åbne kodningsmodeller.

:::note
OpenAiCode Go er i øjeblikket i beta.
:::

Go fungerer ligesom enhver anden udbyder i OpenAiCode. Du abonnerer på OpenAiCode Go og får din API-nøgle. Det er **helt valgfrit**, og du behøver ikke at bruge det for at bruge OpenAiCode.
...
:::tip
Vi testede en udvalgt gruppe af modeller og udbydere, der fungerer godt med OpenAiCode.
:::
...
OpenAiCode Go giver dig adgang til disse modeller for **$10/måned**.

---

## Sådan fungerer det

OpenAiCode Go fungerer ligesom enhver anden udbyder i OpenAiCode.

1. Du logger ind på **<a href={console}>OpenAiCode Zen</a>**, abonnerer på Go, og kopierer din API-nøgle.
2. Du kører kommandoen `/connect` i TUI'en, vælger `OpenAiCode Go`, og indsætter din API-nøgle.
3. Kør `/models` i TUI'en for at se listen over modeller, der er tilgængelige gennem Go.

:::note
Kun ét medlem pr. workspace kan abonnere på OpenAiCode Go.
:::
...
OpenAiCode Go inkluderer følgende grænser:
...
OpenAiCode Go er en **$10/måned** abonnementsplan. Nedenfor er priserne **pr. 1M tokens**.
...
[Model-id'et](/docs/config/#models) i din OpenAiCode-konfiguration bruger formatet `opencode-go/<model-id>`. For eksempel, for Kimi K2.5, ville du bruge `opencode-go/kimi-k2.5` i din konfiguration.
...
Vi skabte OpenAiCode Go for at:
...
4. Have **ingen lock-in** ved at tillade dig også at bruge enhver anden udbyder med OpenAiCode.
