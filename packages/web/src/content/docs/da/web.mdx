---
title: Web
description: Brug af OpenAiCode i din browser.
---

OpenAiCode kan køre som en webapplikation i din browser, hvilket giver den samme kraftfulde AI-kodningsoplevelse uden behov for en terminal.

![OpenAiCode Web - Ny session](../../../assets/web/web-homepage-new-session.png)
...
Dette starter en lokal server på `127.0.0.1` med en tilfældig tilgængelig port og åbner automatisk OpenAiCode i din standardbrowser.
...
Som standard vælger OpenAiCode en tilgængelig port. Du kan angive en port:
...
Som standard binder serveren til `127.0.0.1` (kun lokal vært). Sådan gør du OpenAiCode tilgængelig på dit netværk:

```bash
opencode web --hostname 0.0.0.0
```

Når du bruger `0.0.0.0`, vil OpenAiCode vise både lokale adresser og netværksadresser:
...
Når den er startet, giver webgrænsefladen adgang til dine OpenAiCode-sessioner.

### Sessioner

Se og administrer dine sessioner fra hjemmesiden. Du kan se aktive sessioner og starte nye.

![OpenAiCode Web - Active Session](../../../assets/web/web-homepage-active-session.png)

### Serverstatus

Klik på "Se servere" for at se tilsluttede servere og deres status.

![OpenAiCode Web - Se servere](../../../assets/web/web-homepage-see-servers.png)
