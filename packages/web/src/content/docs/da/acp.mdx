...
description: Brug OpenAiCode i enhver ACP-kompatibel editor.
---

OpenAiCode understøtter [Agent Client Protocol](https://agentclientprotocol.com) eller (ACP), så du kan bruge det direkte i kompatible editorer og IDE'er.
...
For at bruge OpenAiCode via ACP, konfigurer din editor til at køre kommandoen `opencode acp`.

Kommandoen starter OpenAiCode som en ACP-kompatibel underproces, der kommunikerer med din editor over JSON-RPC via stdio.
...
For at åbne den skal du bruge den nye 'OpenAiCode'-agent i AI Chat-agentvælgeren.
...
For at bruge OpenAiCode som en ACP-agent i [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), skal du tilføje følgende til din Neovim-konfiguration:
...
Denne konfiguration sætter CodeCompanion til at bruge OpenAiCode som ACP-agent til chat.
...
OpenAiCode fungerer på samme måde via ACP som i terminalen. Alle funktioner understøtter:
...
- MCP-servere konfigureret i din OpenAiCode-konfiguration
