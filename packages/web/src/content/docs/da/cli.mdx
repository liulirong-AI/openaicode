---
title: CLI
description: OpenAiCode CLI muligheder og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenAiCode CLI starter som standard [TUI](/docs/tui), når den kører uden nogen argumenter.

```bash
opencode
```

Men den accepterer også kommandoer som dokumenteret på denne side. Dette giver dig mulighed for at interagere med OpenAiCode programmatisk.
...
Start OpenAiCode terminalbrugergrænsefladen.
...
Administrer agenter i OpenAiCode.
...
Tilslut en terminal til en allerede kørende OpenAiCode backend-server startet via `serve` eller `web` kommandoer.

```bash
opencode attach [url]
```

Dette gør det muligt at bruge TUI med en ekstern OpenAiCode backend. For eksempel:
...
OpenAiCode drives af udbyderlisten på [Models.dev](https://models.dev), så du kan bruge `opencode auth login` til at konfigurere API-nøgler for enhver udbyder, du vil bruge. Dette er gemt i `~/.local/share/opencode/auth.json`.

```bash
opencode auth login
```

Når OpenAiCode starter op, indlæser den udbyderne fra legitimationsfilen. Og hvis der er nogen nøgler defineret i dine miljøer eller en `.env` fil i dit projekt.
...
Brug flaget `--refresh` til at opdatere den cachelagrede modelliste. Dette er nyttigt, når nye modeller er blevet tilføjet til en udbyder, og du vil se dem i OpenAiCode.
...
Start en hovedløs OpenAiCode-server til API-adgang. Tjek [server docs](/docs/server) for den fulde HTTP-grænseflade.
...
Administrator OpenAiCode sessionsholder.
...
Liste over alle OpenAiCode sessioner.
...
Vis tokenbrug og omkostningsstatistikker for dine OpenAiCode-sessioner.
...
Importer sessionsdata fra en JSON fil eller OpenAiCode del URL.
...
Start en hovedløs OpenAiCode-server med en webgrænseflade.

```bash
opencode web
```

Dette starter en HTTP-server og åbner en webbrowser for at få adgang til OpenAiCode via en webgrænseflade. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `opencode`).
...
Afinstaller OpenAiCode og fjern alle relaterede filer.
...
Opdaterer OpenAiCode til den nyeste version eller en specifik version.
...
OpenAiCode kan konfigureres ved hjælp af miljøvariabler.
