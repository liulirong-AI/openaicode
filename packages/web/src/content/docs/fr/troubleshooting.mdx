...
Pour déboguer les problèmes avec OpenAiCode, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
OpenAiCode Desktop exécute un serveur OpenAiCode local (le side-car `opencode-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `OpenAiCode` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
OpenAiCode peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que OpenAiCode puisse le reconstruire.

1. Quittez complètement OpenAiCode Desktop.
...
3. Redémarrez le bureau OpenAiCode.

---

### Résoudre les problèmes de connexion au serveur

OpenAiCode Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Sur Windows, OpenAiCode Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des performances lentes, des problèmes d'accès aux fichiers ou des problèmes de terminal sur Windows, essayez d'utiliser [WSL (Windows Sous-système pour Linux)](/docs/windows-wsl). WSL fournit un environnement Linux qui fonctionne de manière plus transparente avec les fonctionnalités de OpenAiCode.

---

### Les notifications ne s'affichent pas

OpenAiCode Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour OpenAiCode dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau OpenAiCode.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application OpenAiCode Desktop) :
...
Si vous rencontrez des problèmes avec OpenAiCode :
