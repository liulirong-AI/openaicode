---
title: Web
description: En utilisant OpenAiCode dans votre navigateur.
---

OpenAiCode peut s'exécuter comme une application Web dans votre navigateur, offrant la même expérience puissante de codage d'IA sans avoir besoin d'un terminal.

![OpenAiCode Web - Nouvelle session](../../../assets/web/web-homepage-new-session.png)
...
Cela démarre un serveur local sur `127.0.0.1` avec un port disponible aléatoire et ouvre automatiquement OpenAiCode dans votre navigateur par défaut.
...
Par défaut, OpenAiCode sélectionne un port disponible. Vous pouvez spécifier un port :
...
Par défaut, le serveur se lie à `127.0.0.1` (localhost uniquement). Pour rendre OpenAiCode accessible sur votre réseau :

```bash
opencode web --hostname 0.0.0.0
```

Lorsque vous utilisez `0.0.0.0`, OpenAiCode affichera les adresses locales et réseau :
...
Une fois démarrée, l'interface web permet d'accéder à vos sessions OpenAiCode.
...
![OpenAiCode Web - Session active](../../../assets/web/web-homepage-active-session.png)

### Statut du serveur

Cliquez sur "Voir les serveurs" pour afficher les serveurs connectés et leur état.

![OpenAiCode Web - Voir Serveurs](../../../assets/web/web-homepage-see-servers.png)
