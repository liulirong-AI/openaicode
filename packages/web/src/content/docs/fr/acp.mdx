---
title: Support ACP
description: Utilisez OpenAiCode dans n’importe quel éditeur compatible ACP.
---

OpenAiCode prend en charge le [Agent Client Protocol](https://agentclientprotocol.com) (ACP), vous permettant de l'utiliser directement dans les éditeurs et IDE compatibles.
...
Pour utiliser OpenAiCode via ACP, configurez votre éditeur pour exécuter la commande `opencode acp`.

La commande démarre OpenAiCode en tant que sous-processus compatible ACP qui communique avec votre éditeur via JSON-RPC sur stdio.
...
Pour l'ouvrir, utilisez le nouvel agent « OpenAiCode » dans le sélecteur d'agent AI Chat.
...
Pour utiliser OpenAiCode comme agent ACP dans [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), ajoutez ce qui suit à votre configuration Neovim :
...
Cette configuration configure CodeCompanion pour utiliser OpenAiCode comme agent ACP pour le chat.
...
OpenAiCode fonctionne de la même manière via ACP que dans le terminal. Toutes les fonctionnalités sont prises en charge :
...
- Serveurs MCP configurés dans votre configuration OpenAiCode
