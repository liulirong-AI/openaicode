---
title: Partager
description: Partagez vos conversations OpenAiCode.
---

La fonctionnalité de partage de OpenAiCode vous permet de créer des liens publics vers vos conversations OpenAiCode, afin que vous puissiez collaborer avec vos coéquipiers ou obtenir de l'aide d'autres personnes.
...
Lorsque vous partagez une conversation, OpenAiCode :
...
OpenAiCode prend en charge trois modes de partage qui contrôlent la manière dont les conversations sont partagées :

---

### Manuel (par défaut)

Par défaut, OpenAiCode utilise le mode de partage manuel. Les sessions ne sont pas partagées automatiquement, mais vous pouvez les partager manuellement à l'aide de la commande `/share` :
