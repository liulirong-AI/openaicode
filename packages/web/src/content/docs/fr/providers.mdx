---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en OpenAiCode.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenAiCode utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration OpenAiCode.
...
Vous pouvez personnaliser les fournisseurs via la section `provider` de votre OpenAiCode
configuration.
...
OpenAiCode Zen est une liste de modèles fournis par l'équipe OpenAiCode qui ont été
testé et vérifié pour fonctionner correctement avec OpenAiCode. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par OpenAiCode Zen.
:::
...
Il fonctionne comme n’importe quel autre fournisseur dans OpenAiCode et son utilisation est totalement facultative.

---

## OpenAiCode Go

OpenAiCode Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe OpenAiCode qui ont été
testé et vérifié pour fonctionner correctement avec OpenAiCode.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `OpenAiCode Go` et rendez-vous sur [opencode.ai/auth](https://opencode.ai/zen).
...
Pour utiliser Amazon Bedrock avec OpenAiCode :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans OpenAiCode n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
OpenAiCode utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller OpenAiCode
...
Pour utiliser Google Vertex AI avec OpenAiCode :
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) pour enregistrer automatiquement chaque conversation OpenAiCode en tant que session dans Helicone.
...
Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation OpenAiCode répertoriée comme une session distincte.
...
:::tip
Ollama peut se configurer automatiquement pour OpenAiCode. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/opencode) pour plus de détails.
:::
...
Pour utiliser Ollama Cloud avec OpenAiCode :
...
3. Copiez la clé API à utiliser dans OpenAiCode.
...
6. **Important** : Avant d'utiliser des modèles cloud dans OpenAiCode, vous devez extraire les informations du modèle localement :
...
OpenAiCode Zen est une liste de modèles testés et vérifiés fournie par l'équipe OpenAiCode. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>OpenAiCode Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **OpenAiCode Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé OpenAiCode API.
...
Les champs `limit` permettent à OpenAiCode de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
