...
Il existe deux types d'agents dans OpenAiCode ; agents primaires et sous-agents.
...
OpenAiCode est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
OpenAiCode est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

OpenAiCode est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Si aucune température n'est spécifiée, OpenAiCode utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale OpenAiCode et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration OpenAiCode utilise le format `provider/model-id`. Par exemple, si vous utilisez [OpenAiCode Zen](/docs/zen), vous utiliserez `opencode/gpt-5.1-codex` pour GPT 5.1 Codex.
