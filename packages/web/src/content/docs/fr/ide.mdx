---
title: IDE
description: L'extension OpenAiCode pour VS Code, Cursor et autres IDE
---

OpenAiCode s'intègre à VS Code, Cursor ou tout IDE prenant en charge un terminal. Exécutez simplement `opencode` dans le terminal pour commencer.

---

## Utilisation

- **Lancement rapide** : utilisez `Cmd+Esc` (Mac) ou `Ctrl+Esc` (Windows/Linux) pour ouvrir OpenAiCode dans une vue de terminal divisée, ou focus sur une session de terminal existante si elle est déjà en cours d'exécution.
- **Nouvelle session** : utilisez `Cmd+Shift+Esc` (Mac) ou `Ctrl+Shift+Esc` (Windows/Linux) pour démarrer une nouvelle session de terminal OpenAiCode, même si elle est déjà ouverte. Vous pouvez également cliquer sur le bouton OpenAiCode dans l'interface utilisateur.
- **Contexte** : partagez automatiquement votre sélection ou onglet actuel avec OpenAiCode.
...
Pour installer OpenAiCode sur VS Code et les forks populaires comme Cursor, Windsurf, VSCodium :
...
Recherchez **OpenAiCode** dans Extension Marketplace et cliquez sur **Installer**.
