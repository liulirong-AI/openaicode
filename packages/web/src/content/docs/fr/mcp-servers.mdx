...
Vous pouvez ajouter des outils externes à OpenAiCode à l'aide du _Model Context Protocol_ ou MCP. OpenAiCode prend en charge les serveurs locaux et distants.
...
Vous pouvez définir des serveurs MCP dans votre [OpenAiCode Config](https://opencode.ai/docs/config/) sous `mcp`. Ajoutez chaque MCP avec un nom unique. Vous pouvez faire référence à ce MCP par son nom lorsque vous demandez le LLM.
...
OpenAiCode gère automatiquement l'authentification OAuth pour les serveurs MCP distants. Lorsqu'un serveur nécessite une authentification, OpenAiCode :
...
Si le serveur nécessite une authentification, OpenAiCode vous demandera de vous authentifier lorsque vous essayez de l'utiliser pour la première fois. Sinon, vous pouvez [déclencher manuellement le flux](#authenticating) avec `opencode mcp auth <server-name>`.
...
La commande `mcp auth` ouvrira votre navigateur pour autorisation. Après votre autorisation, OpenAiCode stockera les jetons en toute sécurité dans `~/.local/share/opencode/mcp-auth.json`.
...
Vos MCP sont disponibles sous forme d'outils dans OpenAiCode, aux côtés des outils intégrés. Vous pouvez donc les gérer via la configuration OpenAiCode comme n'importe quel autre outil.
...
Cela ouvrira une fenêtre de navigateur pour terminer le flux OAuth et connecter OpenAiCode à votre compte Sentry.
