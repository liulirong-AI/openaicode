---
title: Zen
description: Liste organisée de modèles fournis par OpenAiCode.
---
...
OpenAiCode Zen est une liste de modèles testés et vérifiés fournie par l'équipe OpenAiCode.

:::note
OpenAiCode Zen est actuellement en version bêta.
:::

Zen fonctionne comme n'importe quel autre fournisseur dans OpenAiCode. Vous vous connectez à OpenAiCode Zen et obtenez votre clé API. C'est **complètement facultatif** et vous n'avez pas besoin de l'utiliser pour utiliser OpenAiCode.
...
:::tip
Nous avons testé un groupe sélectionné de modèles et de fournisseurs qui fonctionnent bien avec OpenAiCode.
:::
...
OpenAiCode Zen est une passerelle IA qui vous donne accès à ces modèles.

---

## Comment ça marche

OpenAiCode Zen fonctionne comme n'importe quel autre fournisseur dans OpenAiCode.

1. Vous vous connectez à **<a href={console}>OpenAiCode Zen</a>**, ajoutez vos informations de facturation et copiez votre clé API.
2. Vous exécutez la commande `/connect` dans le TUI, sélectionnez OpenAiCode Zen et collez votre clé API.
...
Le [id du modèle](/docs/config/#models) dans votre configuration OpenAiCode utilise le format `opencode/<model-id>`. Par exemple, pour GPT 5.2 Codex, vous devez utilisez `opencode/gpt-5.2-codex` dans votre configuration.
...
- MiniMax M2.5 Free est disponible sur OpenAiCode pour une durée limitée. L’équipe profite de ce temps pour recueillir des commentaires et améliorer le modèle.
- Big Pickle est un modèle furtif gratuit sur OpenAiCode pour une durée limitée. L’équipe profite de ce temps pour recueillir des commentaires et améliorer le modèle.
...
Nous avons créé OpenAiCode Zen pour :
...
4. N'ayez **aucun verrouillage** en vous permettant de l'utiliser avec n'importe quel autre agent de codage. Et laissez-vous toujours utiliser n'importe quel autre fournisseur avec OpenAiCode également.
