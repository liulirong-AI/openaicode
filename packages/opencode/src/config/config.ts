...
export namespace Config {
  ...
  export const state = Instance.state(async () => {
    ...
    // Config loading order (low -> high precedence): see docs/config.md
    ...
    for (const [key, value] of Object.entries(auth)) {
      if (value.type === "wellknown") {
        ...
        if (!remoteConfig.$schema) remoteConfig.$schema = "https://openaicode.ai/config.json"
  ...
  export const Info = z
    .object({
      ...
      command: z.record(z.string(), Command).optional().describe("Command configuration, see docs/commands.md"),
      ...
      agent: z
        ...
        .catchall(Agent)
        .optional()
        .describe("Agent configuration, see docs/agents.md"),
      provider: z
    ...
    if (existsSync(legacy)) {
      await import(pathToFileURL(legacy).href, {
        ...
        .then(async (mod) => {
          ...
          result["$schema"] = "https://openaicode.ai/config.json"
  ...
  async function load(text: string, options: { path: string } | { dir: string; source: string }) {
    ...
    if (parsed.success) {
      if (!parsed.data.$schema && isFile) {
        parsed.data.$schema = "https://openaicode.ai/config.json"
        const updated = original.replace(/^\s*\{/, '{\n  "$schema": "https://openaicode.ai/config.json",')
