...
export namespace Provider {
  ...
  const CUSTOM_LOADERS: Record<string, CustomLoader> = {
    ...
    openrouter: async () => {
      return {
        ...
        options: {
          headers: {
            "HTTP-Referer": "https://openaicode.ai/",
            "X-Title": "opencode",
          },
        },
      }
  ...
  const state = Instance.state(async () => {
    ...
    function mergeProvider(providerID: ProviderID, provider: Partial<Info>) {
      ...
      const dbMatch = database[providerID]

      if (existing && dbMatch) {
        const merged = mergeDeep(dbMatch, existing)
        providers[providerID] = mergeDeep(merged, provider) as Info
        return
      }

      if (dbMatch) {
        providers[providerID] = mergeDeep(dbMatch, provider) as Info
      } else if (existing) {
    ...
    for (const [providerID, provider] of configProviders) {
      ...
      for (const [modelID, model] of Object.entries(provider.models ?? {})) {
        ...
        const parsedModel: Model = {
          ...
          limit: {
            context: model.limit?.context ?? existingModel?.limit?.context ?? 32768,
            input: model.limit?.input ?? existingModel?.limit?.input,
            output: model.limit?.output ?? existingModel?.limit?.output ?? 4096,
          },
