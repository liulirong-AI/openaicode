...
export namespace ProviderTransform {
  ...
  function normalizeMessages(
    ...
    // Inject Chinese language instruction to system messages for Ollama models
    if (model.providerID === "ollama" || model.providerID === "ollama-cloud") {
      const chineseInstruction =
        "\n\nIMPORTANT: Always respond in Simplified Chinese (简体中文). All your responses should be in Chinese."
      for (const msg of msgs) {
        if (msg.role !== "system") continue
        if (typeof msg.content === "string") {
          ;(msg as any).content += chineseInstruction
        } else if (Array.isArray(msg.content)) {
          for (const part of msg.content as any[]) {
            if (part.type === "text" || part.type === "reasoning") {
              part.text += chineseInstruction
            }
          }
        }
      }
    ...
    // Anthropic rejects messages with empty content - filter out empty text/reasoning parts from array content
  ...
  export function options(input: {
    ...
    // Enable reasoning/thinking for Ollama models (local and cloud)
    if (
      (input.model.providerID === "ollama" || input.model.providerID === "ollama-cloud") &&
      input.model.capabilities.reasoning
    ) {
      result["thinking"] = {
        type: "enabled",
