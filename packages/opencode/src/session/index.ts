...
const CONTEXT_TOKEN_LIMIT = 200_000
...
export namespace Session {
  ...
  export const getUsage = fn(
    ...
    (input) => {
      ...
      const metadata = input.metadata ?? ({} as Record<string, Record<string, unknown>>)
      // Access provider-specific cache token fields
      const bedrockMeta = metadata["bedrock"] as { usage?: Record<string, number> } | undefined
      const veniceMeta = metadata["venice"] as { usage?: Record<string, number> } | undefined
      const cacheWriteInputTokens = safe(
        (metadata["anthropic"]?.["cacheCreationInputTokens"] ??
          bedrockMeta?.["usage"]?.["cacheWriteInputTokens"] ??
          veniceMeta?.["usage"]?.["cacheCreationInputTokens"] ??
          0) as number,
      )

      const excludesCached = !!(metadata["anthropic"] || metadata["bedrock"])
      const adjustedInputTokens = safe(
        excludesCached ? inputTokens : inputTokens - cacheReadInputTokens - cacheWriteInputTokens,
      )
      ...
      const costInfo =
        input.model.cost?.experimentalOver200K && tokens.input + tokens.cache.read > CONTEXT_TOKEN_LIMIT
          ? input.model.cost.experimentalOver200K
          : input.model.cost
      return {
        cost: safe(
          new Decimal(0)
            .add(new Decimal(tokens.input).mul(costInfo?.input ?? 0).div(MILLION_DIVISOR))
            .add(new Decimal(tokens.output).mul(costInfo?.output ?? 0).div(MILLION_DIVISOR))
            .add(new Decimal(tokens.cache.read).mul(costInfo?.cache?.read ?? 0).div(MILLION_DIVISOR))
            .add(new Decimal(tokens.cache.write).mul(costInfo?.cache?.write ?? 0).div(MILLION_DIVISOR))
            .add(new Decimal(tokens.reasoning).mul(costInfo?.output ?? 0).div(MILLION_DIVISOR))
            .toNumber(),
        ),
        tokens,
      }
