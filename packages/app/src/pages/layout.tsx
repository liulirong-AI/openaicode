...
export default function Layout(props: ParentProps) {
  ...
  const [state, setState] = createStore({
    ...
    peeked: false,
    skillStatus: "idle" as "idle" | "running" | "error",
  })
  ...
  // Sidebar project - determines which project to show in sidebar
  const sidebarProject = createMemo(() => {
    if (layout.sidebar.opened()) {
      const current = currentProject()
      if (current) return current
      // When sidebar is opened but no project in URL, show first available project
      const projects = layout.projects.list()
  ...
  const visibleSessionDirs = createMemo(() => {
    // Use sidebarProject for sidebar display, currentProject for main content
    const project = sidebarProject() || currentProject()
    if (!project || !project.worktree) return []
    if (!workspaceSetting()) return [project.worktree]

    const activeDir = currentDir()
    const ids = workspaceIds(project)
    if (!ids || !Array.isArray(ids)) return [project.worktree]
    return ids.filter((directory) => {
  ...
  function openSkillsSettings() {
    dialog.show(() => <DialogSettings initialTab="skills" />)
  }
        ...
        <Show
          ...
          fallback={
            <div class="flex flex-col items-center justify-center h-full text-center p-4">
              <div class="text-14-regular text-text-weak">Select a project</div>
            </div>
          }
        >
          <>
  ...
  const sidebarContent = (mobile?: boolean) => (
    <SidebarContent
      ...
      onOpenHelp={() => platform.openLink("/docs/README.html")}
      onOpenSkills={openSkillsSettings}
      skillStatus={() => state.skillStatus ?? "idle"}
      ...
      renderPanel={() =>
        mobile ? (
          ...
          <Show when={sidebarProject() || layout.sidebar.opened()}>
        ...
        <div class="flex-1 min-h-0 relative">
          <div class="size-full relative overflow-x-hidden">
            <nav
              ...
              onMouseLeave={() => {
                aim.reset()
                // Disable auto-close - sidebar stays open when explicitly opened by user click
                // Only auto-close for hover-opened sidebar
